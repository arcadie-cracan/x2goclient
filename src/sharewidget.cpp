...
ShareWidget::ShareWidget(QString id, ONMainWindow *mw, QWidget *parent, Qt::WindowFlags f)
    : ConfigWidget(id, mw, parent, f)
{
    ...
    QPushButton *openDir = new QPushButton(QIcon(mainWindow->iconsPath("/16x16/file-open.png")),
                                           QString(),
                                           egb);
    ...
    cbFsConv = new QCheckBox(tr("Filename encoding"), egb);
    ...
    cbFsSshTun = new QCheckBox(tr("Use SSH port forwarding to tunnel file system "
    ...
    expTv->header()->resizeSection(1, fm1.horizontalAdvance(tr("Automount")) + 10);
    ...
    connect(cbFsConv, SIGNAL(clicked()), this, SLOT(slot_convClicked()));
    readConfig();
}
...
void ShareWidget::slot_openDir()
{
    ...
    QString path = QFileDialog::getExistingDirectory(this, tr("Select folder"), startDir);
    if (path != QString()) {
#ifdef Q_OS_WIN
        if (ONMainWindow::getPortable() && ONMainWindow::U3DevicePath().length() > 0) {
...
void ShareWidget::readConfig()
{
    ...
    QString exportDir = st.setting()->value(sessionId + "/export", (QVariant) QString()).toString();

    cbFsSshTun->setChecked(st.setting()->value(sessionId + "/fstunnel", true).toBool());
    QStringList lst = exportDir.split(";", Qt::SkipEmptyParts);

    QString toCode = st.setting()->value(sessionId + "/iconvto", (QVariant) "UTF-8").toString();

#ifdef Q_OS_WIN
    QString fromCode
        = st.setting()->value(sessionId + "/iconvfrom", (QVariant) tr("WINDOWS-1252")).toString();
#endif
#ifdef Q_OS_DARWIN
    QString fromCode = st.setting()->value(sessionId + "/iconvfrom", (QVariant) "UTF-8").toString();
#endif
#ifdef Q_OS_LINUX
    QString fromCode
        = st.setting()->value(sessionId + "/iconvfrom", (QVariant) tr("ISO8859-1")).toString();
#endif

    cbFsConv->setChecked(st.setting()->value(sessionId + "/useiconv", (QVariant) false).toBool());
    slot_convClicked();
...
{
    X2goSettings st("sessions");
    st.setting()->setValue(sessionId + "/fstunnel", (QVariant) cbFsSshTun->isChecked());
