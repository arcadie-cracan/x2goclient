...
void ONMainWindow::initUI()
{
    ...
    if (ONMainWindow::portable) {
        ...
        x2goInfof(2) << tr("Starting X2Go Client in portable mode. Data directory is: ")
                     << portableDataPath;
    ...
    if (readExportsFrom != QString()) {
        ...
        connect(exportTimer, SIGNAL(timeout()), this, SLOT(slotExportTimer()));
    }
    if (extLogin) {
    ...
    if (QGuiApplication::screens().at(wd.screenNumber(this))->geometry().width() < 1024
        || QGuiApplication::screens().at(wd.screenNumber(this))->geometry().height() < 768) {
        miniMode = true;
    ...
    if (bg_dir.isDir()) {
        ...
        int random_selector = QRandomGenerator::global()->generate() % max;
    ...
    SVGFrame* x2g = new SVGFrame((QString) x2gologopath, false, fr);
    ...
    act_set = new QAction(QIcon(iconsPath("/32x32/edit_settings.png")), tr("&Settings ..."), this);

    if (supportMenuFile != QString()) {
        act_support = new QAction(tr("Support ..."), this);
        connect(act_support, SIGNAL(triggered(bool)), this, SLOT(slotSupport()));
    }

    act_abclient = new QAction(QIcon(iconsPath("/32x32/x2goclient.png")),
                               tr("About X2Go Client"),
                               this);

    connect(act_set, SIGNAL(triggered(bool)), this, SLOT(slotConfig()));
    connect(act_abclient, SIGNAL(triggered(bool)), this, SLOT(slotAbout()));
...
#ifdef Q_OS_WIN
    QProcess::startDetached("x2gohelper.exe", (QStringList() << QString::number(GetCurrentProcessId())));
    ...
    connect(fr, SIGNAL(resized(const QSize)), this, SLOT(slotResize(const QSize)));
    slotResize(fr->size());
    ...
    if (brokerMode) {
        ...
        connect(broker, SIGNAL(fatalHttpError()), this, SLOT(close()));
        ...
        connect(broker,
                SIGNAL(enableBrokerLogoutButton()),
                this,
                SLOT(slotEnableBrokerLogoutButton()));
        brokerAliveTimer = new QTimer(this);
        connect(brokerAliveTimer, SIGNAL(timeout()), this, SLOT(slotSendBrokerAlive()));
    ...
    connect(proxyWinTimer, SIGNAL(timeout()), this, SLOT(slotFindProxyWin()));
...
bool ONMainWindow::get_translator(QString file_name_start, QTranslator** translator)
{
    ...
    for (QStringList::const_iterator it = ui_languages.constBegin(); it != ui_languages.constEnd();
         ++it) {
        ...
        if ((*it).startsWith("en")) {
            ...
            if (tmp_translator->load(load_filename)) {
                ...
                *translator = tmp_translator;
                ...
                x2goWarningf(1) << tr("Non-fatal: can't load translator: ")
                                       + load_filename.toLatin1();
                x2goWarningf(1) << tr(
...
void ONMainWindow::initWidgetsNormal()
{
    ...
    act_edit = new QAction(QIcon(iconsPath("/32x32/edit.png")), tr("Session management ..."), this);
    ...
    if (changeBrokerPass) {
        act_changeBrokerPass = new QAction(QIcon(iconsPath("/32x32/auth.png")),
                                           tr("&Set broker password ..."),
                                           this);
        connect(act_changeBrokerPass, SIGNAL(triggered(bool)), this, SLOT(slotChangeBrokerPass()));
        act_changeBrokerPass->setEnabled(false);
    }

    if (connTest) {
        act_testCon = new QAction(QIcon(iconsPath("/32x32/contest.png")),
                                  tr("&Connectivity test ..."),
                                  this);
        connect(act_testCon, SIGNAL(triggered(bool)), this, SLOT(slotTestConnection()));
    }
    ...
    connect(act_abqt, SIGNAL(triggered(bool)), this, SLOT(slotAboutQt()));
    connect(act_new, SIGNAL(triggered(bool)), this, SLOT(slotNewSession()));
    connect(act_sessicon, SIGNAL(triggered(bool)), this, SLOT(slotCreateSessionIcon()));
    connect(act_edit, SIGNAL(triggered(bool)), this, SLOT(slotManage()));
    connect(act_exit, SIGNAL(triggered(bool)), this, SLOT(trayQuit()));
...
void ONMainWindow::slotGetBrokerAuth()
{
    ...
    if (config.brokerNoAuth)
        slotSessEnter();
    else if (config.brokerurl.indexOf("ssh://") == 0
             && (config.brokerAutologin || config.brokerKrbLogin
                 || config.brokerSshKey.length() > 0))
        slotSessEnter();
}
            ...
            if (sessionStatusDlg && sessionStatusDlg->isVisible()) {
                if (!useLdap) {
                    if (!(sessionExplorer->getLastSession())) {
                        ...
                        trayIconActiveConnectionMenu->setTitle(
                            sessionExplorer->getLastSession()->name());
                    }
                } else
...
#else
            trayIcon->setToolTip(tr("A left click hides or restores the window. A right click "
        ...
        if (users->width() != usize) {
            ...
            if (useLdap) {
                ...
                for (fit = sessionExplorer->getFoldersList()->begin(); fit != fend; fit++) {
                    if (!miniMode)
                        (*fit)->move((usize - 360) / 2, (*fit)->pos().y());
                    else
                        (*fit)->move((usize - 250) / 2, (*fit)->pos().y());
                }
...
void ONMainWindow::closeClient()
{
    ...
    if (!startMaximized && !startHidden && !embedMode) {
        ...
        st.setting()->setValue("mainwindow/size", QVariant(size()));
        st.setting()->setValue("mainwindow/pos", QVariant(pos()));
        st.setting()->setValue("mainwindow/maximized", QVariant(isMaximized()));
        st.setting()->sync();
...
#if defined(Q_OS_DARWIN) || defined(Q_OS_WIN)
    if (pulseManager) {
        disconnect(pulseManager,
                   SIGNAL(sig_pulse_user_warning(bool, const QString&, const QString&, bool)),
                   this,
                   SLOT(slotShowPAMSGDialog(bool, const QString&, const QString&, bool)));
        delete (pulseManager);
...
{
    X2goSettings st("sizes");
    mwSize = st.setting()->value("mainwindow/size", (QVariant) QSize(800, 600)).toSize();
    mwPos = st.setting()->value("mainwindow/pos", (QVariant) QPoint(20, 20)).toPoint();
    mwMax = st.setting()->value("mainwindow/maximized", (QVariant) false).toBool();
    ...
    for (auto &screen: QGuiApplication::screens()) {
        QRect tmp_geom = screen->availableGeometry();
    ...
    if (!ldapOnly) {
        useLdap = st1.setting()->value("LDAP/useldap", (QVariant) false).toBool();
        ldapServer = st1.setting()->value("LDAP/server", (QVariant) "localhost").toString();
        ldapPort = st1.setting()->value("LDAP/port", (QVariant) 389).toInt();
        ldapDn = st1.setting()->value("LDAP/basedn", (QVariant) QString()).toString();
        ldapServer1 = st1.setting()->value("LDAP/server1", (QVariant) QString()).toString();
        ldapPort1 = st1.setting()->value("LDAP/port1", (QVariant) 0).toInt();
        ldapServer2 = st1.setting()->value("LDAP/server2", (QVariant) QString()).toString();
        ldapPort2 = st1.setting()->value("LDAP/port2", (QVariant) 0).toInt();
    }
    showToolBar = st1.setting()->value("toolbar/show", (QVariant) true).toBool();
}
...
void ONMainWindow::slotPassEnter()
{
    ...
    for (; it != end; ++it) {
        ...
        QString sPort = "22";
        list<string> sL = LDAPSession::getStringAttrValues(*it, "l");
        if (sL.size() > 0) {
...
void ONMainWindow::slotConfig()
{
    ...
    {
        X2goSettings st("settings");
        oldDisableInput = st.setting()->value("pulse/norecord", (QVariant) false).toBool();
    }
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        if (!systemDisablePA) {
            X2goSettings st("settings");
            bool disablePA = st.setting()->value("pulse/disable", (QVariant) false).toBool();
            ...
            } else if (pulseManager
                       && !disablePA) //pulse is already inited and not disabled by config dialog
            {
                bool newDisableInput
                    = st.setting()->value("pulse/norecord", (QVariant) false).toBool();
...
void ONMainWindow::slotReadSessions()
{
    ...
    if (brokerMode) {
        ...
        sessionStatusDlg->hide();
        setEnabled(true);
        if (config.brokerSyncTimeout * 1000 != static_cast<unsigned int>(brokerSyncTimer->interval())) {
        ...
        if (defaultSessionId.length() > 0) {
            for (int i = 0; i < sessionExplorer->getSessionsList()->size(); ++i) {
                if (sessionExplorer->getSessionsList()->at(i)->id() == defaultSessionId) {
                    sessionExplorer->setCurrrentPath(
                        sessionExplorer->getSessionsList()->at(i)->getPath());
            ...
            for (int i = 0; i < sessionExplorer->getSessionsList()->size(); ++i) {
                QString normalName = (sessionExplorer->getSessionsList()->at(i)->getPath() + "/"
                                      + sessionExplorer->getSessionsList()->at(i)->name());
...
bool ONMainWindow::initLdapSession(bool showError)
{
        ...
        if (ldapServer1.length()) {
            try {
                ...
                if (ldapServer2.length()) {
                    try {
                        ld = new LDAPSession(ldapServer2.toStdString(),
                                             ldapPort2,
                                             "",
                                             "",
                                             true,
    ...
    if (res.size() != 0) {
        LDAPStringEntry entry = res.front();
        list<string> str = LDAPSession::getStringAttrValues(entry, NETSOUNDSYSTEM);
        if (str.size()) {
        ...
        if (str.size()) {
            LDAPSndStartServer = (str.front() == "yes") ? true : false;
        }
    }
#endif
...
void ONMainWindow::slotSelectedFromList(SessionButton* session)
{
    ...
    if (!embedMode) {
        ...
        sessIcon = wrap_legacy_resource_URIs(
            st->setting()
                ->value(sid + "/icon", (QVariant) iconsPath("/128x128/x2gosession.png"))
                .toString());
        sessIcon = expandHome(sessIcon);

        command = st->setting()->value(sid + "/command", (QVariant) tr("KDE")).toString();

        server = st->setting()->value(sid + "/host", (QVariant) QString()).toString();
        userName = st->setting()->value(sid + "/user", (QVariant) QString()).toString();
        if (defaultUser && userName.length() < 1)
            userName = defaultUserName;

        if (brokerMode)
            usebrokerpass = st->setting()->value(sid + "/usebrokerpass", false).toBool();

        sshPort = st->setting()->value(sid + "/sshport", (QVariant) defaultSshPort).toString();
        currentKey = st->setting()->value(sid + "/key", (QVariant) QString()).toString();
        currentKey = expandHome(currentKey);

        autologin = st->setting()->value(sid + "/autologin", (QVariant) false).toBool();
#ifdef Q_OS_LINUX
        if (command == "XDMCP"
            && st->setting()->value(sid + "/directxdmcp", (QVariant) false).toBool())
            autologin = true;
#endif
        krblogin = st->setting()->value(sid + "/krblogin", (QVariant) false).toBool();
        delete st;
#ifdef Q_OS_WIN
        if (portable && u3Device.length() > 0) {
    ...
    QString text = "<b>" + sessionName + "</b><br>" + command + tr(" on ") + server;
    nameLabel->setText(text);
    ...
    if (embedMode) {
        QTimer::singleShot(50, this, SLOT(slotShowPassForm()));
    }
}

SshMasterConnection* ONMainWindow::startSshConnection(QString host,
                                                      QString port,
                                                      bool acceptUnknownHosts,
                                                      QString login,
                                                      QString password,
                                                      bool autologin,
                                                      bool krbLogin,
                                                      ...
                                                      bool proxyKrbLogin)
{
    ...
    x2goDebug << "Starting new ssh connection to server:" << host << ":" << port
              << " krbLogin: " << krbLogin;
    ...
    con = new SshMasterConnection(this,
                                  host,
                                  port.toInt(),
                                  acceptUnknownHosts,
                                  login,
    ...
    qRegisterMetaType<SshMasterConnection::passphrase_types>(
        "SshMasterConnection::passphrase_types");

    connect(con,
            SIGNAL(serverAuthError(int, QString, SshMasterConnection*)),
            this,
    ...
    connect(con,
            ...
            SLOT(slotSshServerAuthPassphrase(SshMasterConnection*,
                                             SshMasterConnection::passphrase_types)));
    connect(con,
            SIGNAL(needChallengeResponse(SshMasterConnection*, QString)),
            this,
    ...
    connect(con,
            SIGNAL(connectionError(QString, QString)),
            this,
            SLOT(slotSshConnectionError(QString, QString)));
    connect(con,
            SIGNAL(startInteraction(SshMasterConnection*, QString)),
            this,
            SLOT(slotSshInteractionStart(SshMasterConnection*, QString)));
    connect(con,
            SIGNAL(updateInteraction(SshMasterConnection*, QString)),
            this,
            SLOT(slotSshInteractionUpdate(SshMasterConnection*, QString)));
    connect(con,
            SIGNAL(finishInteraction(SshMasterConnection*)),
            this,
...
void ONMainWindow::slotSshConnectionError(QString message, QString lastSessionError)
{
    ...
    if (!startHidden) {
        QMessageBox::critical(0l, message, lastSessionError, QMessageBox::Ok, QMessageBox::NoButton);
        setEnabled(true);
...
void ONMainWindow::slotSshServerAuthPassphrase(SshMasterConnection* connection,
                                               SshMasterConnection::passphrase_types passphrase_type)
{
    bool ok;
    QString message;
    ...
    if (ok) {
        QString phrase = QInputDialog::getText(0,
...
void ONMainWindow::slotSshServerAuthChallengeResponse(SshMasterConnection* connection,
                                                      QString challenge)
{
    bool ok = false;
    QString message;
...
void ONMainWindow::slotSshServerAuthError(int error,
                                          QString sshMessage,
...
void ONMainWindow::slotSessEnter()
{
    ...
    if (brokerMode) {
        ...
        QString cmd = st->setting()->value(sid + "/command", (QVariant) QString()).toString();
        directRDP = (st->setting()->value(sid + "/directrdp", (QVariant) false).toBool()
                     && cmd == "RDP");
        ...
        directRDP = (st->setting()->value(sid + "/directxdmcp", (QVariant) false).toBool()
                     && cmd == "RDP");
...
void ONMainWindow::continueNormalSession()
{
    ...
    if (!shadowSession)
        sshConnection->executeCommand("x2golistsessions",
                                      this,
                                      SLOT(slotListSessions(bool, QString, int)));
    else
        sshConnection->executeCommand("x2golistdesktops",
                                      this,
                                      SLOT(slotListSessions(bool, QString, int)));
}
...
void ONMainWindow::startDirectRDP()
{
    ...
    bool fullscreen
        = st->setting()->value(sid + "/fullscreen", (QVariant) defaultFullscreen).toBool();
    bool maxRes = st->setting()->value(sid + "/maxdim", (QVariant) false).toBool();
    int height = st->setting()->value(sid + "/height", (QVariant) defaultHeight).toInt();
    int width = st->setting()->value(sid + "/width", (QVariant) defaultWidth).toInt();
    QString sessionType = st->setting()->value(sid + "/command", (QVariant) "").toString();

    QString client = st->setting()->value(sid + "/rdpclient", (QVariant) "rdesktop").toString();
    if (client == "xfreerdpnew") {
        client = "xfreerdp";
        freeRDPNew = true;
    }
    QString host = st->setting()->value(sid + "/host", (QVariant) "").toString();
    QString port = st->setting()->value(sid + "/rdpport", (QVariant) "3389").toString();
    QString params = st->setting()->value(sid + "/directrdpsettings", (QVariant) "").toString();
    ...
    if ((!brokerMode) || (brokerCredsForSession)) {
        ...
        user = st->setting()->value(sid + "/user", (QVariant) "").toString();
    }
    ...
    connect(nxproxy, SIGNAL(readyReadStandardError()), this, SLOT(slotProxyStderr()));
    connect(nxproxy, SIGNAL(readyReadStandardOutput()), this, SLOT(slotProxyStdout()));
    ...
    if (!freeRDPNew) {
        ...
        if (fullscreen) {
            ...
            QDesktopWidget wd;
            grOpt = " -D -g " + QString::number(wd.screenGeometry().width()) + "x"
                    + QString::number(wd.screenGeometry().height()) + " ";
        } else {
        ...
        if (fullscreen) {
            ...
            QDesktopWidget wd;
            grOpt = " /w:" + QString::number(wd.screenGeometry().width())
                    + " /h:" + QString::number(wd.screenGeometry().height()) + " ";
        } else {
    ...
    if (sessionType == "XDMCP") {
        ...
        client = st->setting()->value(sid + "/xdmcpclient", (QVariant) "Xnest").toString();
        params = st->setting()->value(sid + "/directxdmcpsettings", (QVariant) "").toString();
        if (client == "Xephyr") {
            if (fullscreen) {
                grOpt = " -fullscreen ";
            } else if (maxRes) {
                QDesktopWidget wd;
                grOpt = " -screen " + QString::number(wd.screenGeometry().width()) + "x"
                        + QString::number(wd.screenGeometry().height()) + " ";
            } else {
            ...
            if (maxRes) {
                QDesktopWidget wd;
                grOpt = " -geometry " + QString::number(wd.screenGeometry().width()) + "x"
                        + QString::number(wd.screenGeometry().height()) + " ";
            } else {
...
#else
    resumingSession.sessionId += QString::number(QDateTime::currentDateTime().toMSecsSinceEpoch()
                                                 / 1000);
#endif
...
bool ONMainWindow::startSession(const QString& sid, CONTYPE conType)
{
    ...
    if (brokerMode) {
        ...
        host = st->setting()->value(sid + "/host", (QVariant) QString()).toString();
    }

    QString cmd = st->setting()->value(sid + "/command", (QVariant) QString()).toString();
    autologin = st->setting()->value(sid + "/autologin", (QVariant) false).toBool();
    krblogin = st->setting()->value(sid + "/krblogin", (QVariant) false).toBool();
    krbDelegation = st->setting()->value(sid + "/krbdelegation", (QVariant) false).toBool();
#ifdef Q_OS_LINUX
    directRDP = (st->setting()->value(sid + "/directrdp", (QVariant) false).toBool()
                 && cmd == "RDP");
    ...
    if (!embedMode) {
        useproxy = (st->setting()->value(sid + "/usesshproxy", false).toBool());

        QString prtype = (st->setting()->value(sid + "/sshproxytype", "SSH").toString());
        ...
        proxylogin = (st->setting()->value(sid + "/sshproxyuser", QString()).toString());

        proxyKey = (st->setting()->value(sid + "/sshproxykeyfile", QString()).toString());
        proxyKey = expandHome(proxyKey);

        proxyserver = (st->setting()->value(sid + "/sshproxyhost", QString()).toString());

        proxyport = (st->setting()->value(sid + "/sshproxyport", 22).toInt());

        proxyAutologin = (st->setting()->value(sid + "/sshproxyautologin", false).toBool());

        proxyKrbLogin = (st->setting()->value(sid + "/sshproxykrblogin", false).toBool());
    } else {
    ...
    bool proxySamePass = (st->setting()->value(sid + "/sshproxysamepass", false).toBool());
    bool proxySameUser(st->setting()->value(sid + "/sshproxysameuser", false).toBool());
    bool proxySameKey = (st->setting()->value(sid + "/sshproxysamekey", false).toBool());
    ...
    if (proxySamePass)
        ...
        if (useproxy && !proxyAutologin && !proxyKrbLogin && proxyKey.length() <= 0) {
            ...
            if (brokerMode) {
                useBrokerPassForProxy
                    = (st->setting()->value(sid + "/usebrokerpassforproxy", false).toBool());
                useBrokerUserForProxy
                    = (st->setting()->value(sid + "/usebrokeruserforproxy", false).toBool());
            ...
            if (useBrokerPassForProxy)
                ...
                proxypassword = QInputDialog::getText(0,
                                                      proxylogin + "@" + proxyserver + ":"
                                                          + QString::number(proxyport),
                                                      tr("Enter password for SSH proxy"),
                                                      QLineEdit::Password,
                                                      QString(),
                                                      &ok);
        }
    ...
    sshConnection = startSshConnection(host,
                                       sshPort,
                                       acceptRsa,
                                       user,
                                       passwd,
...
void ONMainWindow::slotListSessions(bool result, QString output, int)
{
    interDlg->hide();
    x2goDebug << output;
    if (result == false) {
    ...
    if (shadowSession) {
        ...
        if ((sessions.size() == 0) || (sessions.size() == 1 && sessions[0].length() < 5))
            startNewSession();
            ...
            if ((s.agentPid != "invalid") && (s.status == "S")
                && (isColorDepthOk(wd.depth(), s.colorDepth)) && (s.command == selectedCommand)
                && autoresume)
                resumeSession(s);
            else {
...
void ONMainWindow::startNewSession()
{
    ...
    if (useLdap) {
        ...
        if (!embedMode) {
            ...
            sid = "embedded";
        pack = st->setting()->value(sid + "/pack", (QVariant) defaultPack).toString();
        fullscreen = st->setting()->value(sid + "/fullscreen", (QVariant) defaultFullscreen).toBool();
        ...
        if (st->setting()->value(sid + "/maxdim", (QVariant) false).toBool()) {
            ...
            height = QGuiApplication::screens().at(selectedScreen)->availableGeometry().height();
            width = QGuiApplication::screens().at(selectedScreen)->availableGeometry().width();
            ...
            height = st->setting()->value(sid + "/height", (QVariant) defaultHeight).toInt();
            width = st->setting()->value(sid + "/width", (QVariant) defaultWidth).toInt();
        }

        setDPI = st->setting()->value(sid + "/setdpi", (QVariant) defaultSetDPI).toBool();
        dpi = st->setting()->value(sid + "/dpi", (QVariant) defaultDPI).toUInt();
        quality = st->setting()->value(sid + "/quality", (QVariant) defaultQuality).toInt();
        speed = st->setting()->value(sid + "/speed", (QVariant) defaultLink).toInt();

        xinerama = st->setting()
                       ->value(sid + "/xinerama", static_cast<QVariant>(defaultXinerama))
                       .toBool();

        clipMode
            = st->setting()->value(sid + "/clipboard", (QVariant) defaultClipboardMode).toString();

        usekbd = st->setting()->value(sid + "/usekbd", (QVariant) defaultSetKbd).toBool();
        layout = st->setting()->value(sid + "/layout", (QVariant) defaultLayout[0]).toString();
        type = st->setting()->value(sid + "/type", (QVariant) defaultKbdType).toString();
        type.replace("\\", "");
        type.replace("(", "\\(");
        type.replace(")", "\\)");
        if (!embedMode) {
            command = st->setting()->value(sid + "/command", (QVariant) defaultCmd).toString();

            kdrive = st->setting()->value(sid + "/kdrive", (QVariant) false).toBool();

            host = st->setting()->value(sid + "/host", (QVariant) (QString) "localhost").toString();

            rootless = st->setting()->value(sid + "/rootless", (QVariant) false).toBool();
            resumingSession.published
                = st->setting()->value(sid + "/published", (QVariant) false).toBool();
            xdmcpServer
                = st->setting()->value(sid + "/xdmcpserver", (QVariant) "localhost").toString();
        } else {
            command = config.command;
            if (command == "SHADOW") {
                shadowSession = true;
            ...
            if (st->setting()->value(sid + "/startembed", (QVariant) true).toBool()) {
                startEmbedded = true;
                fullscreen = false;
        ...
        uint displays = QGuiApplication::screens().size();
    ...
    if (!fullscreen) {
        ...
        if (embedMode) {
            ...
            geometry += "+" + QString::number(position.x()) + "+"
                        + QString::number(position.y() + stb->height());
        }
    }
    QString link;
    switch (speed) {
    ...
    QString cmd = dpiEnv + xdmcpEnv + xinerama_env + " x2gostartagent " + geometry + " " + link
                  + " " + pack + " unix-kde-depth_" + depth + " " + layout + " " + type + " ";
    if (usekbd)
        cmd += "1 ";
    else
        cmd += "0 ";
    ...
    if (!shadowSession) {
        ...
        cmd += sessTypeStr + QString::number(shadowMode) + "XSHAD" + shadowUser + "XSHAD"
               + shadowDisplay;
    }
    ...
    sshConnection->executeCommand(cmd, this, SLOT(slotRetResumeSess(bool, QString, int)));
    passForm->hide();
}
...
void ONMainWindow::resumeSession(const x2goSession& s)
{
    ...
    if (useLdap) {
        ...
        if (brokerMode) {
            ...
            host = st->setting()->value(sid + "/host", (QVariant) s.server).toString();
        }
        delete st;
    }
...
// #endif
    ...
    QString cmd = "x2goresume-session " + s.sessionId + " " + geometry + " " + link + " " + pack
                  + " " + layout + " " + type + " ";
    if (usekbd)
        cmd += "1";
    else
...
void ONMainWindow::setTrayIconToSessionIcon(QString info)
{
    ...
    if (trayIcon && sessionExplorer->getLastSession()) {
        ...
        if (!keepTrayIcon) {
            QString imagePath = wrap_legacy_resource_URIs(expandHome(
                st->setting()
                    ->value(sid + "/icon", (QVariant) QString(iconsPath("/128x128/x2go.png")))
                    .toString()));
    ...
    if (!shadowSession) {
        ...
        for (int row = 0; row < sessions.size(); ++row) {
            ...
            for (int j = 0; j < 8; ++j) {
                ...
                if (sessTv->header()->sectionSize(j) < fm.horizontalAdvance(txt) + 6) {
                    sessTv->header()->resizeSection(j, fm.horizontalAdvance(txt) + 6);
                }
            }
        }
    } else {
        ...
        if (!embedMode) {
            ...
            if (!brokerMode) {
                if (!(sessionExplorer->getLastSession())) {
                    x2goDebug
                        << "No session selected, returning without starting a shadow session.";
                    return;
                }
                ...
                QString sid = sessionExplorer->getLastSession()->id();
                QString suser
                    = st->setting()->value(sid + "/shadowuser", (QVariant) QString()).toString();
                QString sdisplay
                    = st->setting()->value(sid + "/shadowdisplay", (QVariant) QString()).toString();
                bool fullAccess
                    = st->setting()->value(sid + "/shadowfullaccess", (QVariant) false).toBool();
...
void ONMainWindow::slotActivated(const QModelIndex& index)
{
    if (!shadowSession) {
        ...
        QString user = sessTv->model()->index(index.row(), D_USER).data().toString();
    ...
    if (!useLdap) {
        if (!brokerMode) {
            if (embedMode) {
                ...
                QString sid = sessionExplorer->getLastSession()->id();
                host = st.setting()->value(sid + "/host", (QVariant) host).toString();
            }
        }
    } else {
        sshConnection = findServerSshConnection(host);
...
void ONMainWindow::slotRetSuspSess(bool result, QString output, int)
{
    setEnabled(true);
    if (result == false) {
        QString message = tr("<b>Connection failed.</b>\n") + output;
        ...
        if (selectSessionDlg->isVisible()) {
            ((QStandardItemModel*) (sessTv->model()))
                ->item(sessTv->currentIndex().row(), S_STATUS)
                ->setData(QVariant((QString) tr("suspended")), Qt::DisplayRole);
            bSusp->setEnabled(false);
            sOk->setEnabled(true);
        }
    }
...
void ONMainWindow::slotRetTermSess(bool result, QString output, int)
{
    setEnabled(true);
    if (result == false) {
        QString message = tr("<b>Connection failed.</b>\n") + output;
...
void ONMainWindow::slotRetResumeSess(bool result, QString output, int)
{
    ...
    if (useLdap) {
        ...
        if (!embedMode) {
            if (!(sessionExplorer->getLastSession())) {
                x2goDebug << "No session selected, this should not happen since we already started "
                             "resuming a session.";
                return;
            }
        ...
        kdrive = st->setting()->value(sid + "/kdrive", (QVariant) false).toBool();
        sound = st->setting()->value(sid + "/sound", (QVariant) true).toBool();
        QString sndsys = st->setting()->value(sid + "/soundsystem", (QVariant) "pulse").toString();
        if (sndsys == "arts")
            sndSystem = ARTS;
        if (sndsys == "esd")
            sndSystem = ESD;
...
#endif /* !defined (Q_OS_WIN) && !defined (Q_OS_DARWIN) */
        startSoundServer = st->setting()->value(sid + "/startsoundsystem", true).toBool();
...
#if !defined(Q_OS_WIN) && !defined(Q_OS_DARWIN)
        bool defPort = st->setting()->value(sid + "/defsndport", true).toBool();
        if (defPort) {
            switch (sndSystem) {
            case PULSE:
                sndPort = "4713";
...
#endif /* !defined (Q_OS_WIN) && !defined (Q_OS_DARWIN) */
        sshSndTunnel = st->setting()->value(sid + "/soundtunnel", true).toBool();
            ...
            if (!pulseManager || !(pulseManager->is_server_running())) {
                ...
                if (!systemDisablePA && !disablePA) {
                    show_RichText_WarningMsgBox(
                        tr("PulseAudio is not running"),
                        tr("Sound output will be disabled for this session. Please enable "
                           "PulseAudio in the configuration dialog or disable sound in the session "
                           "settings"),
                        true);
                }
                sound = false;
    ...
    if (newSession) {
        ...
        if (managedMode) {
            ...
            config.sessiondata = resumingSession.agentPid + "|" + resumingSession.sessionId + "|"
                                 + resumingSession.display + "|" + resumingSession.server + "|"
                                 + "S|" + resumingSession.crTime + "|" + resumingSession.cookie
                                 + "|" + resumingSession.clientIp + "|" + resumingSession.grPort
                                 + "|" + resumingSession.sndPort + "|" + resumingSession.crTime
                                 + "|" + user + "|" + "0|" + resumingSession.fsPort;
        }
    ...
    while (iport == resumingSession.sndPort.toInt() || iport == resumingSession.fsPort.toInt()
           || isServerRunning(iport))
        ++iport;
    localGraphicPort = QString::number(iport);

    sshConnection->startTunnel("localhost",
                               resumingSession.grPort.toInt(),
                               "localhost",
                               localGraphicPort.toInt(),
                               false,
                               this,
                               SLOT(slotTunnelOk(int)),
                               SLOT(slotTunnelFailed(bool, QString, int)));
    if (shadowSession)
        return;
    ...
    if (sound) {
        if (sndSystem == PULSE) {
            ...
            if (!sshSndTunnel)
                scmd = "echo \"default-server=$(echo "
                       ...
                       + sndPort + "\" > \"${HOME}/.x2go/C-" + resumingSession.sessionId
                       + "/.pulse-client.conf\""
                         ";echo \"cookie-file=${HOME}/.x2go/C-"
                       + resumingSession.sessionId + "/.pulse-cookie" + "\" >> \"${HOME}/.x2go/C-"
                       + resumingSession.sessionId
                       + "/.pulse-client.conf\""
                         ";echo \"autospawn=no\" >>"
                         "\"${HOME}/.x2go/C-"
                       + resumingSession.sessionId + "/.pulse-client.conf\"";
            else
                scmd = "echo \"default-server=localhost:" + resumingSession.sndPort
                       + "\" > \"${HOME}/.x2go/C-" + resumingSession.sessionId
                       + "/.pulse-client.conf\""
                         ";echo \"cookie-file=${HOME}/.x2go/C-"
                       + resumingSession.sessionId + "/.pulse-cookie" + "\" >> \"${HOME}/.x2go/C-"
                       + resumingSession.sessionId
            ...
            if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
                while (!file.atEnd()) {
                    ...
                    int pos = line.indexOf("PULSEAUDIO_SYSTEM_START=1");
                    if (pos != -1) {
            ...
            if (sysPulse) {
                ...
                if (pulsecookie_filename.length() > 0) {
                    sshConnection->copyFile(pulsecookie_filename,
                                            "$HOME/.x2go/C-" + resumingSession.sessionId
                                                + "/.pulse-cookie",
                                            this,
                                            SLOT(slotPCookieReady(bool, QString, int)));
                }
            } else {
#if !defined(Q_OS_WIN) && !defined(Q_OS_DARWIN)
                ...
                QString cooFile = QDir::toNativeSeparators(
                    QDir(pulseManager->get_pulse_dir().absolutePath() + "/.pulse-cookie")
                        .absolutePath());
                QString destFile = "$HOME/.x2go/C-" + resumingSession.sessionId + "/.pulse-cookie";
        ...
        if (sndSystem == ESD) {
#if !defined(Q_OS_WIN) && !defined(Q_OS_DARWIN)
            sshConnection->copyFile(homeDir + "/.esd_auth", "$HOME/.esd_auth");
#else  /* !defined (Q_OS_WIN) && !defined (Q_OS_DARWIN) */
            QString cooFile = QDir::toNativeSeparators(
                QDir(pulseManager->get_pulse_dir().absolutePath() + "/.esd_auth").absolutePath());
            QString destFile = "$HOME/.esd_auth";
            sshConnection->copyFile(cooFile, destFile);
#endif /* defined (Q_OS_LINUX) */
        }
/* Windows and Darwin are covered by PulseManager. */
        ...
        if (startSoundServer) {
            ...
            if (sndSystem == ESD)
                soundServer->start(ecmd + " -tcp -nobeeps -bind localhost -port "
                                   + resumingSession.sndPort);
            if (sndSystem == ARTS)
                soundServer->start(acmd + " -u -N -p " + resumingSession.sndPort);
            sndPort = resumingSession.sndPort;
        }
#endif /* !defined (Q_OS_WIN) && !defined (Q_OS_DARWIN) */
        if (sshSndTunnel) {
            sndTunnel = sshConnection->startTunnel("localhost",
                                                   resumingSession.sndPort.toInt(),
                                                   "127.0.0.1",
                                                   sndPort.toInt(),
                                                   true,
                                                   this,
                                                   NULL,
                                                   SLOT(slotSndTunnelFailed(bool, QString, int)));
        }
    }
}
...
void ONMainWindow::slotTunnelOk(int)
{
    ...
    if (!d.exists())
        if (!d.mkpath(dirpath)) {
            ...
            QMessageBox::critical(0l,
                                  tr("Error"),
                                  tr("Unable to create folder: ") + dirpath,
                                  QMessageBox::Ok,
                                  QMessageBox::NoButton);
            slotShowPassForm();
            //             if ( tunnel )
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QMessageBox::critical(0l,
                              tr("Error"),
                              tr("Unable to write file: ") + dirpath + "/options",
                              QMessageBox::Ok,
                              QMessageBox::NoButton);
        slotShowPassForm();
        return;
...
#endif
    out << "nx/nx,root=" << nxroot << ",connect=localhost,cookie=" << resumingSession.cookie
        << ",port=" << localGraphicPort /*resumingSession.grPort*/ << ",errors=" << dirpath
        << "/sessions:" << resumingSession.display;
    file.close();
    xmodExecuted = false;

    qDebug() << "RESUMING SESSION is KDRIVE: "
             << (resumingSession.sessionType == x2goSession::KDRIVE);
    ...
    env.insert(0, "PATH=/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin:/usr/X11/bin:/usr/X11R6/bin");
    ...
    QStringList proxyOptions;
    proxyOptions << "-S"
                 << "nx/nx,root=" + dirpath
                 << "nx/nx,options=" + dirpath + "/options:" + resumingSession.display;
    ...
    if (resumingSession.sessionType == x2goSession::KDRIVE) {
        ...
        if (!embedMode) {
            if (!useLdap) {
                ...
                if (brokerMode)
                    ...
                    st = new X2goSettings("sessions");
                randr = st->setting()->value(sid + "/xinerama", (QVariant) defaultXinerama).toBool();
        ...
        proxyOptions.clear();
        proxyOptions << "--connect"
                     << "localhost"
                     << "--port" << localGraphicPort << "--title" << resumingSession.sessionId << "-S"
                     << "nx/nx,options=" + dirpath + "/options:" + resumingSession.display
                     << "--selection" << clipboard;
        if (randr) {
            proxyOptions << "--randr";
        ...
        if (fs) {
            ...
            proxyOptions << "--screen" << dispNumber;
        } else {
            proxyOptions << "--width" << width << "--height" << height;
        }

        x2goDebug << "Starting NX proxy, command: " + proxyCmd << proxyOptions.join(" ");
        nxproxy->start(proxyCmd, proxyOptions);
    } else {
        x2goDebug << "Starting NX proxy, command: " + proxyCmd << proxyOptions.join(" ");
        nxproxy->start(proxyCmd, proxyOptions);
    }
...
void ONMainWindow::slotTunnelFailed(bool result, QString output, int)
{
    if (result == false) {
        if (!managedMode) {
...
void ONMainWindow::slotSndTunnelFailed(bool result, QString output, int)
{
    if (result == false) {
        if (!managedMode) {
            QString message = tr("Unable to create SSH tunnel for audio data:\n") + output;
            QMessageBox::warning(0l, tr("Warning"), message, QMessageBox::Ok, QMessageBox::NoButton);
        }
        sndTunnel = 0l;
    }
}
...
void ONMainWindow::slotSetModMap()
{
    ...
    if (kbMap.isEmpty()) {
        ...
        if (path_val.isEmpty()) {
            ...
            path_val = MACPORTS_PREFIX "/bin:" MACPORTS_PREFIX
                                       "/sbin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin:/usr/"
                                       "local/sbin:/usr/X11R6/bin:/opt/X11/bin";
        ...
        if (!key_map_fetch_ret) {
            ...
            if (!mod_fetch_ret) {
                ...
                for (int i = 0; i < lines.count(); ++i) {
                    ...
                    QString mod = parts[0];
                    if ((mod == "shift") || (mod == "lock") || (mod == "control") || (mod == "mod1")
                        || (mod == "mod2") || (mod == "mod3") || (mod == "mod4")
                        || (mod == "mod5")) {
    ...
    if (kbMap.isEmpty()) {
        x2goWarningf(9) << "Could not fetch keyboard map via xmodmap. Internal error. Not "
                           "synchronizing with remote side.";
    } else {
        /* Send modified map to server. */
        QString cmd = "export DISPLAY=\":" + resumingSession.display + "\"; echo \"" + kbMap
                      + "\" | xmodmap -";
...
void ONMainWindow::handle_xmodmap_error(QProcess& proc)
{
    ...
    informative_text += tr("X2Go Client will now terminate.\n\n"
                           "File a bug report as outlined on the <a "
                           "href=\"http://wiki.x2go.org/doku.php/wiki:bugs\">bugs wiki page</a>.");
...
void ONMainWindow::slotProxyError(QProcess::ProcessError err)
{
    ...
    if (err == QProcess::FailedToStart && directRDP) {
        ...
        QString informative_text = tr(
            "Check session settings and ensure that selected client is installed on your system.");

        show_RichText_ErrorMsgBox(main_text, informative_text);
    }
#endif
    x2goDebug << "Proxy command crashed!";
...
void ONMainWindow::slotProxyFinished(int, QProcess::ExitStatus)
{
    ...
    disconnect(nxproxy,
               ...
               this,
               SLOT(slotProxyFinished(int, QProcess::ExitStatus)));
    disconnect(nxproxy, SIGNAL(readyReadStandardError()), this, SLOT(slotProxyStderr()));
    disconnect(nxproxy, SIGNAL(readyReadStandardOutput()), this, SLOT(slotProxyStdout()));
    proxyRunning = false;
    if (trayEnabled) {
    ...
    if (directRDP) {
        ...
        pass->setText("");
        QTimer::singleShot(2000, this, SLOT(slotShowPassForm()));
        return;
    }
#endif
    if (!emergencyExit && !shadowSession && !usePGPCard
        && !(embedMode && (config.checkexitstatus == false))) {
        ...
        if (startHidden) {
            ...
            x2goInfof(10) << tr(
                "Closing X2Go Client because the --close-disconnect parameter was passed.");
            close();
        }
    }
    ...
    if (!restartResume) {
        if (brokerMode && (!config.brokerAutologoff)) {
            ...
            QTimer::singleShot(2000, broker, SLOT(getUserSessions()));
...
void ONMainWindow::slotProxyStderr()
{
    ...
    if (stInfo->toPlainText().indexOf("Connecting to remote host 'localhost:" +
                                      /*resumingSession.grPort*/ localGraphicPort)
        != -1) {
        setStatStatus(tr("connecting"));
    }
    ...
    if (stInfo->toPlainText().indexOf("Established X server connection") != -1) {
        ...
        disconnect(sbSusp, SIGNAL(clicked()), this, SLOT(slotTestSessionStatus()));
        ...
        if (newSession) {
            ...
            if (!xmodExecuted) {
                xmodExecuted = true;
                QTimer::singleShot(2000, this, SLOT(slotExecXmodmap()));
            }
        }
#endif
    }
...
void ONMainWindow::slotShowAdvancedStat()
{
    if (!miniMode) {
        if (sbAdv->isChecked()) {
            sessionStatusDlg->setFixedSize(sessionStatusDlg->width(),
    ...
    X2goSettings st("settings");
    st.setting()->setValue("showStatus", (QVariant) sbAdv->isChecked());
    st.setting()->sync();
}
...
void ONMainWindow::termBrokerSession(const QString& sessId, const QString& host)
{
    if (QMessageBox::warning(this,
                             tr("Warning"),
                             tr("Are you sure you want to terminate "
                                "this session?\n"
...
{
    sshConnection->executeCommand("x2gosuspend-session " + sessId,
                                  this,
                                  SLOT(slotRetSuspSess(bool, QString, int)));
}
...
bool ONMainWindow::termSession(QString sessId, bool warn)
{
    ...
    sshConnection->executeCommand("x2goterminate-session " + sessId,
                                  this,
                                  SLOT(slotRetTermSess(bool, QString, int)));
    proxyRunning = false;
    return true;
}
...
void ONMainWindow::setStatStatus(QString status)
{
    ...
    if (!embedMode || !proxyWinEmbedded) {
        ...
        QString message = getCurrentUname() + "@" + srv + ", " + tr("Session") + ": "
                          + resumingSession.sessionId + ", " + tr("Display") + ": "
                          + resumingSession.display + ", " + tr("Creation time") + ": " + tstr;
        if (statusLabel) {
...
{
#ifdef Q_OS_WIN
    if (!embedMode || !config.confSnd || (config.confSnd && config.useSnd))
#endif /* defined (Q_OS_WIN) */
    {
        X2goSettings st("settings");
        bool disablePulse = st.setting()->value("pulse/disable", (QVariant) false).toBool();
        ...
        connect(pulseManager,
                SIGNAL(sig_pulse_user_warning(bool, const QString&, const QString&, bool)),
                this,
                SLOT(slotShowPAMSGDialog(bool, const QString&, const QString&, bool)));

        bool disableInput = st.setting()->value("pulse/norecord", (QVariant) false).toBool();
...
void ONMainWindow::runCommand()
{
    if (runRemoteCommand) {
         ...
         * Fixes: #1100
         */
        sshConnection->executeCommand("x2gobasepath",
                                      this,
                                      SLOT(SlotRunCommand(bool, QString, int)),
                                      true);
    }
#ifdef Q_WS_HILDON
...
{
    QString cmd = "PULSE_CLIENTCONFIG=\"${HOME}/.x2go/C-" + resumingSession.sessionId
                  + "/.pulse-client.conf\" DISPLAY=:" + resumingSession.display + " setsid " + exec
...
void ONMainWindow::SlotRunCommand(bool, QString output, int)
{
    ...
    if (!embedMode) {
        ...
        if (useLdap)
            ...
            QString sid = sessionExplorer->getLastSession()->id();
            command = st->setting()->value(sid + "/command", (QVariant) tr("KDE")).toString();
            rdpOpts = st->setting()->value(sid + "/rdpoptions", (QVariant) "").toString();
            if (user != "") {
            ...
            rdpServer = st->setting()->value(sid + "/rdpserver", (QVariant) "").toString();
            rootless = st->setting()->value(sid + "/rootless", (QVariant) false).toBool();
            resumingSession.published
                = st->setting()->value(sid + "/published", (QVariant) false).toBool();

            rdpFS = st->setting()->value(sid + "/fullscreen", (QVariant) defaultFullscreen).toBool();
            rdpHeight = st->setting()->value(sid + "/height", (QVariant) defaultHeight).toString();
            rdpWidth = st->setting()->value(sid + "/width", (QVariant) defaultWidth).toString();
        }
        delete st;
    } else {
        command = config.command;
    ...
    if (sshConnection->useKerberos() && sshConnection->get_kerberosDelegation()) {
        ...
        KRBFL=$HOME/.x2go/C-"
                      + resumingSession.sessionId + "/krb5cc ;\
        cp -a $KRB5CCNAME $KRBFL;KRB5CCNAME=$KRBFL ";
    }

    if (!startSessSound || startSessSndSystem == PULSE) {
        cmd = krbFwString + "setsid " + output + "/bin/x2goruncommand " + resumingSession.display
              + " " + resumingSession.agentPid + " " + resumingSession.sessionId + " "
              + resumingSession.sndPort + " " + command + " nosnd " + sessionType
              + " 1> /dev/null 2>/dev/null & exit";
        if (startSessSndSystem == PULSE) {
            cmd = "export PULSE_CLIENTCONFIG=\"${HOME}/.x2go/C-" + resumingSession.sessionId
        ...
        case ESD:
            cmd = krbFwString + "setsid " + output + "/bin/x2goruncommand "
                  + resumingSession.display + " " + resumingSession.agentPid + " "
                  + resumingSession.sessionId + " " + resumingSession.sndPort + " " + command
                  + " esd " + sessionType + " 1> /dev/null 2>/dev/null & exit";
            break;
        case ARTS:
            cmd = krbFwString + "setsid " + output + "/bin/x2goruncommand "
                  + resumingSession.display + " " + resumingSession.agentPid + " "
                  + resumingSession.sessionId + " " + resumingSession.sndPort + " " + command
                  + " arts " + sessionType + " 1> /dev/null 2>/dev/null & exit";
            break;
        }
    }

    sshConnection->executeCommand(cmd, this, SLOT(slotRetRunCommand(bool, QString, int)), false);
}

void ONMainWindow::slotRetRunCommand(bool result, QString output, int)
{
    if (result == false) {
        QString message = tr("<b>Connection failed.</b>\n:\n") + output;
        if (message.indexOf("publickey,password") != -1) {
...
{
    sshConnection->executeCommand("x2gogetapps",
                                  this,
                                  SLOT(slotReadApplications(bool, QString, int)));
    sbApps->setEnabled(false);
}

void ONMainWindow::slotReadApplications(bool result, QString output, int)
{
    if (result == false) {
        QString message = tr("<b>Connection failed.</b>\n:\n") + output;
        if (message.indexOf("publickey,password") != -1) {
    ...
    foreach (QString appstr, output.split("</desktop>", Qt::SkipEmptyParts)) {
        ...
        for (int i = 0; i < lines.count(); ++i) {
            ...
            if (line.indexOf("Name[" + localshort + "]=") != -1
                || line.indexOf("Name[" + locallong + "]=") != -1) {
            ...
            if (line.indexOf("Comment[" + localshort + "]=") != -1
                || line.indexOf("Comment[" + locallong + "]=") != -1) {
            ...
            if (line.indexOf("<icon>") != -1) {
                ...
                while (line.indexOf("</icon>") == -1) {
                    ...
                    line = lines[++i];
                    if (QString(QByteArray::fromBase64(line.toLatin1()))
                            .indexOf("</svg>", Qt::CaseInsensitive)
                        != -1) {
                        isSvg = true;
                    }
                }
    ...
    if (runStartApp && autostartApps.length() > 0) {
        for (int i = 0; i < autostartApps.length(); ++i) {
            ...
            if (!startAppFound) {
                x2goDebug << "Autostart application " << autostartApps[i]
                          << " not found in desktop files.";
            } else {
...
bool ONMainWindow::parseParameter(QString param)
{
    ...
    if ("--xserver-start-limit" == setting) {
        ...
        if (conv_ret) {
            ...
            x2goDebug << "Conversion for --xserver-start-limit value " << value
                      << " failed; assuming default of 3.";
...
bool ONMainWindow::linkParameter(QString value)
{
    if (value == "modem")
        ...
        printError(tr("Invalid value for parameter \"--link\".").toLocal8Bit().data());
        return false;
    }
    return true;
}
...
bool ONMainWindow::clipboardParameter(QString value)
{
    if (value == "both" || value == "client" || value == "server" || value == "none") {
        ...
        printError(tr("Invalid value for parameter \"--clipboard\".").toLocal8Bit().data());
        return false;
    }
}
...
bool ONMainWindow::geometry_par(QString val)
{
    if (val == "fullscreen")
        ...
        if (res.size() != 2) {
            printError(tr("Invalid value for parameter \"--geometry\".").toLocal8Bit().data());
            return false;
        }
        bool o1, o2;
...
bool ONMainWindow::setKbd_par(QString val)
{
    if (val == "1")
        ...
        printError(tr("Invalid value for parameter \"--set-kbd\".").toLocal8Bit().data());
        return false;
    }
    return true;
}
...
bool ONMainWindow::ldapParameter(QString val)
{
    ...
    if (lst.size() != 3) {
        printError(tr("Invalid value for parameter \"--ldap\".").toLocal8Bit().data());
        return false;
    }
    ldapOnly = true;
    ldapServer = lst[0];
...
bool ONMainWindow::ldap1Parameter(QString val)
{
    ...
    QStringList lst = ldapstring.split(':', Qt::SkipEmptyParts);
    if (lst.size() != 2) {
        printError(tr("Invalid value for parameter \"--ldap1\".").toLocal8Bit().data());
        return false;
    }
    ldapServer1 = lst[0];
...
bool ONMainWindow::ldap2Parameter(QString val)
{
    ...
    QStringList lst = ldapstring.split(':', Qt::SkipEmptyParts);
    if (lst.size() != 2) {
        printError(tr("Invalid value for parameter \"--ldap2\".").toLocal8Bit().data());
        return false;
    }
    ldapServer2 = lst[0];
...
{
    qCritical("%s", tr("Available pack methods:").toLocal8Bit().data());
    QFile file(":/txt/packs");
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))
        return;
    QTextStream in(&file);
...
void ONMainWindow::slotGetServers(bool result, QString output, int)
{
    if (result == false) {
        cardReady = false;
        cardStarted = false;
        ...
        if (lst.size() > 1) {
            for (int j = 0; j < x2goServers.size(); ++j)
                if (x2goServers[j].name == lst[0]) {
                    ...
                    x2goDebug << x2goServers[j].name << ": sessions " << lst[1].toInt()
    ...
    for (int j = 0; j < x2goServers.size(); ++j) {
        ...
        serverSshConnections << startSshConnection(host,
                                                   sshPort,
                                                   acceptRsa,
                                                   user,
                                                   passwd,
...
void ONMainWindow::slotListAllSessions(bool result, QString output, int)
{
    bool last = false;
...
void ONMainWindow::slotExportDirectory()
{
    ...
    if (!useLdap && !embedMode) {
        ...
        path = QFileDialog::getExistingDirectory(this, QString(), homeDir);
    if (hide_after)
        hide();
#ifdef Q_OS_WIN
...
void ONMainWindow::exportDirs(QString exports, bool removable)
{
    ...
    if (!useLdap) {
        if (!embedMode) {
            ...
            fsInTun = st.setting()->value(sid + "/fstunnel", (QVariant) true).toBool();
        } else
            fsInTun = true;
    }
    if (fsInTun) {
...
void ONMainWindow::exportDefaultDirs()
{
    ...
    if (!useLdap) {
        if (!embedMode) {
            ...
            clientPrinting = st->setting()
                                 ->value(sessionExplorer->getLastSession()->id() + "/print", true)
                                 .toBool();

            QString exd = st->setting()
                              ->value(sessionExplorer->getLastSession()->id() + "/export",
                                      (QVariant) QString())
                              .toString();
            QStringList lst = exd.split(";", Qt::SkipEmptyParts);
            for (int i = 0; i < lst.size(); ++i) {
                ...
                QStringList tails = lst[i].split(splitChar, Qt::SkipEmptyParts);
                if (tails.length() != 2) {
                    //wrong format
                    continue;
                }
...
#ifdef Q_OS_WIN
                    tails[0] = cygwinPath(wapiShortFileName(tails[0]));
#endif
                    dirs += tails[0];
                }
            }
    ...
    if (clientPrinting) {
        QString path = homeDir + "/.x2go/S-" + resumingSession.sessionId + "/spool";
        QDir spooldir;
        if (!spooldir.exists(path)) {
            if (!spooldir.mkpath(path)) {
                QString message = tr("Unable to create directory:") + path;
                QMessageBox::critical(0l,
                                      tr("Error"),
                                      message,
                                      QMessageBox::Ok,
        ...
        connect(spoolTimer, SIGNAL(timeout()), this, SLOT(slotCheckPrintSpool()));
...
void ONMainWindow::slotRetExportDir(bool result, QString output, int pid)
{
    ...
    if (result == false) {
        QString message = tr("<b>Connection failed.</b>\n") + output;
        x2goDebug << "startX2goMount failed to mount client-side folder, reason: " << message
                  << Qt::endl;
        if (message.indexOf("publickey,password") != -1) {
            message = tr("<b>Wrong password!</b><br><br>") + message;
        }
...
void ONMainWindow::slotExtTimer()
{
    if (QFile::permissions(readLoginsFrom)
        != (QFile::ReadUser | QFile::WriteUser | QFile::ExeUser | QFile::ReadOwner
            | QFile::WriteOwner | QFile::ExeOwner)) {
        x2goDebug << "Wrong permissions on " << readLoginsFrom << ":";
        x2goDebug << (int) (QFile::permissions(readLoginsFrom + "/.")) << "must be"
                  << (int) (QFile::ReadUser | QFile::WriteUser | QFile::ExeUser | QFile::ReadOwner
                            | QFile::WriteOwner | QFile::ExeOwner)
                  << Qt::endl;
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (!file.atEnd()) {
            QByteArray line = file.readLine();
            QString ln(line);
            QStringList args = ln.split("=", Qt::SkipEmptyParts);
            if (args.size() > 1) {
                if (args[0] == "login") {
                    args[1].replace("\n", "");
                    if (args[1].size())
...
{
    if (QFile::permissions(readExportsFrom)
        != (QFile::ReadUser | QFile::WriteUser | QFile::ExeUser | QFile::ReadOwner
            | QFile::WriteOwner | QFile::ExeOwner)) {
        x2goDebug << "Wrong permissions on " << readExportsFrom << ":" << Qt::endl;
        x2goDebug << (int) (QFile::permissions(readExportsFrom + "/.")) << "must be"
                  << (int) (QFile::ReadUser | QFile::WriteUser | QFile::ExeUser | QFile::ReadOwner
                            | QFile::WriteOwner | QFile::ExeOwner)
                  << Qt::endl;
        exportTimer->stop();
    ...
    for (int i = 0; i < args.size(); ++i) {
        sshConnection->executeCommand("x2goumount_session " + sessionId + " " + args[i]);
    }
}
...
English version: (C) 20AB-20CD Trans Lator <trans@la.tor><br>
    ...
    QString aboutStr = tr("<br>(C) 2005-2020 by <b>obviously nice</b>: "
                          "Oleksandr Shneyder, Heinz-Markus Graesing<br>");
    aboutStr += tr("<br>This is a client to access the X2Go network-based "
    ...
    QMessageBox::about(this,
                       tr("About X2Go Client"),
                       tr("<b>X2Go Client v. ") + VERSION + "</b> (Qt - " + qVersion() + ")"
                           + aboutStr);
}
...
void ONMainWindow::slotStartPGPAuth()
{
    ...
    connect(scDaemon, SIGNAL(readyReadStandardError()), this, SLOT(slotScDaemonStdErr()));
    connect(scDaemon, SIGNAL(readyReadStandardOutput()), this, SLOT(slotScDaemonStdOut()));
    connect(scDaemon,
            SIGNAL(finished(int, QProcess::ExitStatus)),
            this,
            SLOT(slotScDaemonFinished(int, QProcess::ExitStatus)));
    connect(scDaemon,
            SIGNAL(error(QProcess::ProcessError)),
            this,
            SLOT(slotScDaemonError(QProcess::ProcessError)));
    scDaemon->start(QString("pcsc_scan"), (QStringList() << "-n"));
}
...
void ONMainWindow::slotShowPAMSGDialog(bool error,
...
void ONMainWindow::slotGpgFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == 0) {
        ...
        if (login == "[not set]" || authKey == "[none]") {
            ...
            QMessageBox::critical(0l,
                                  tr("Error"),
                                  tr("This card is unknown to the X2Go system."),
                                  QMessageBox::Ok,
                                  QMessageBox::NoButton);
        } else {
            cardReady = true;
...
void ONMainWindow::startGPG()
{
    ...
    connect(gpg, SIGNAL(readyReadStandardError()), this, SLOT(slotGpgError()));
    connect(gpg,
            SIGNAL(finished(int, QProcess::ExitStatus)),
            this,
            SLOT(slotGpgFinished(int, QProcess::ExitStatus)));
...
void ONMainWindow::slotExecXmodmap()
{
    ...
    SshProcess* xmodProc;
    try {
        xmodProc = new SshProcess(this, user, host, sshPort, cmd, passwd, currentKey, acceptRsa);
...
void ONMainWindow::check_cmd_status()
{
    ...
    sshConnection->executeCommand("x2gocmdexitmessage " + resumingSession.sessionId,
                                  this,
                                  SLOT(slotCmdMessage(bool, QString, int)));
}

void ONMainWindow::slotCmdMessage(bool result, QString output, int)
{
...
int ONMainWindow::startSshFsTunnel()
{
    ...
    fsTunnel = sshConnection->startTunnel("localhost",
                                          resumingSession.fsPort.toUInt(),
                                          "127.0.0.1",
                                          clientSshPort.toInt(),
                                          true,
                                          this,
                                          SLOT(slotFsTunnelOk(int)),
                                          SLOT(slotFsTunnelFailed(bool, QString, int)));
    return 0;
}

void ONMainWindow::slotFsTunnelFailed(bool result, QString output, int)
{
    if (result == false) {
        if (!managedMode) {
            QString message
                = tr("Unable to create SSH tunnel for Folder Sharing and Printing support:\n")
                  + output;
            QMessageBox::critical(0l, tr("Error"), message, QMessageBox::Ok, QMessageBox::NoButton);
        }
        fsTunnel = 0l;
...
void ONMainWindow::startX2goMount()
{
    ...
    if (!authorized_keys_file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
        QString message = tr("Unable to create or append to file: ")
    ...
    if (authorized_keys_file_perm != authorized_keys_file_target_perm) {
        if (!authorized_keys_file.setPermissions(authorized_keys_file_target_perm)) {
            ...
            QString message = tr("Unable to change the permissions of file: ")
                              + authorized_keys_file.fileName();
            message += "\n" + tr("This is an error because sshd would deny such a file.");
            QMessageBox::critical(NULL, tr("Error"), message, QMessageBox::Ok, QMessageBox::NoButton);
            permission_error = true;
        }
    }

    QFile::Permissions authorized_keys_dir_perm = QFile(authorized_keys_dir.absolutePath())
                                                      .permissions();
    QFile::Permissions authorized_keys_dir_target_perm = QFile::ReadOwner | QFile::WriteOwner
                                                         | QFile::ExeOwner;
    ...
    if (authorized_keys_dir_perm != authorized_keys_dir_target_perm) {
        if (!QFile(authorized_keys_dir.absolutePath())
                 .setPermissions(authorized_keys_dir_target_perm)) {
            /* FIXME: use a function for this... */
            QString message = tr("Unable to change the permissions of directory: ")
                              + authorized_keys_dir.absolutePath();
    ...
    if (!rem)
        cmd = "x2gomountdirs dir " + sessionId + " \"" + cuser + "\" " + dir->dstKey + " " + dirs;
    else
        cmd = "x2gomountdirs rem " + sessionId + " \"" + cuser + "\" " + dir->dstKey + " " + dirs;
    ...
    if (!useLdap) {
        ...
        if (!embedMode) {
            ...
            sid = "embedded";
        if (st.setting()->value(sid + "/useiconv", (QVariant) false).toBool()) {
            QString toCode = st.setting()->value(sid + "/iconvto", (QVariant) "UTF-8").toString();

#ifdef Q_OS_WIN
            QString fromCode
                = st.setting()->value(sid + "/iconvfrom", (QVariant) tr("WINDOWS-1252")).toString();
#endif
#ifdef Q_OS_DARWIN
            QString fromCode = st.setting()->value(sid + "/iconvfrom", (QVariant) "UTF-8").toString();
#endif
#ifdef Q_OS_LINUX
            QString fromCode
                = st.setting()->value(sid + "/iconvfrom", (QVariant) tr("ISO8859-1")).toString();
#endif
            cmd = "export X2GO_ICONV=modules=iconv,from_code=" + fromCode + ",to_code=" + toCode
                  + "&&" + cmd;
        }
    }

    x2goDebug << "Calling startX2goMount command." << Qt::endl;
    dir->pid = sshConnection->executeCommand(cmd, this, SLOT(slotRetExportDir(bool, QString, int)));
}
...
bool ONMainWindow::isServerRunning(int port)
{
    ...
    InetPton(AF_INET, "127.0.0.1", &addr.s_addr);
    ...
    InetPton(AF_INET, localIP, &saServer.sin_addr.s_addr);
...
void ONMainWindow::startXOrg(int start_offset)
{
    ...
    if (useInternalX && (internalX == XMING)) {
        ...
        xorgLogFile = wapiShortFileName(xorgLogFile) + "\\xorg.log." + QString::number(xDisplay);
        if (QFile::exists(xorgLogFile))
            QFile::remove(xorgLogFile);
        xorgLogMutex.unlock();
        //run xming with clipboard support
        args << dispString << "-multiwindow"
             << "-notrayicon"
             << "-clipboard"
             << "-logfile" << xorgLogFile;
    ...
    if (isServerRunning(6000 + xDisplay)) {
        ...
        if (waitingForX > 10) {
            ...
            if ((!(x_start_limit_)) || (x_start_limit_ >= x_start_tries_)) {
                 ...
                 * should kill it.
                 */
                x2goDebug << "Timeout reached waiting for the X.Org Server to open a listening TCP "
                             "socket."
                          << Qt::endl
...
WinServerStarter::WinServerStarter(daemon server, ONMainWindow* par)
    ...
    , mode(server)
    , parent(par)
    , ssh_key_type_(ONMainWindow::RSA_KEY_TYPE)
{}
...
void ONMainWindow::startWinServers(ONMainWindow::key_types key_type)
{
    ...
    WinServerStarter* xStarter = new WinServerStarter(WinServerStarter::X, this);
    WinServerStarter* sshStarter = new WinServerStarter(WinServerStarter::SSH, this);
    ...
    if (useInternalX && (internalX == XMING)) {
        ...
        connect(xorgLogTimer, SIGNAL(timeout()), this, SLOT(slotCheckXOrgLog()));
...
void ONMainWindow::xorgSettings()
{
    ...
    xorgWinOptions
        = (st.setting()->value("optionswin", "-screen 0 %wx%h -notrayicon -clipboard").toString());
    xorgFSOptions
        = (st.setting()->value("optionsfs", "-fullscreen -notrayicon -clipboard").toString());
    xorgSAppOptions
        = (st.setting()->value("optionssingle", "-multiwindow -notrayicon -clipboard").toString());
    xorgMDOptions = (st.setting()
                         ->value("optionswholedisplay",
                                 "-nodecoration -notrayicon -clipboard -screen 0 @")
                         .toString());
...
void ONMainWindow::generateEtcFiles()
{
    ...
    if (sftp_binary.isEmpty()) {
        x2goErrorf(31) << "Unable to find the sftp-server binary. Neither bundled, nor found in "
                          "$PATH nor additional directories.";
        show_RichText_ErrorMsgBox(
            tr("Unable to find the sftp-server binary. Neither bundled, nor found in $PATH nor "
               "additional directories."),
            tr("If you are using a Linux-based operating system, please ask your system "
               "administrator "
               "to install the package containing the sftp-server binary. Common names are "
               "<b>openssh</b>, "
               "<b>openssh-server</b> or <b>openssh-sftp-server</b> depending upon "
               "distribution.\n\n"
...
ONMainWindow::key_types ONMainWindow::check_key_type(ONMainWindow::key_types key_type)
{
    ...
    if (UNKNOWN_KEY_TYPE == ret) {
        QMessageBox::critical(this,
                              tr("SSH key type selection error"),
...
QString ONMainWindow::generateKey(ONMainWindow::key_types key_type, bool host_key)
{
...

    {
        ...
        if (!(dir.mkpath(base_dir))) {
            QMessageBox::critical(this,
                                  tr("SSH key base directory creation error"),
    ...
    if ((!(QFile::exists(private_key_file))) || (!(QFile::exists(public_key_file)))) {
        ...
        args << "-t" << stringified_key_type << "-b" << QString::number(key_bits) << "-N"
             << ""
             << "-C" << comment << "-f" << private_key_file;

        const int keygen_ret = QProcess::execute("ssh-keygen", args);

        if (-2 == keygen_ret) {
            QMessageBox::critical(this,
                                  tr("ssh-keygen launching error"),
...
QString ONMainWindow::createKeyBundle(key_types key_type)
{
   ...
   */
    ...
    if (!(rsa.open(QIODevice::ReadOnly | QIODevice::Text))) {
        ...
        if (!(rsa.open(QIODevice::ReadOnly | QIODevice::Text))) {
            x2goErrorf(9) << tr("Unable to open newly generated %1 public host key file.")
                                 .arg(stringified_key_type.toUpper());
            return (QString());
        }
#else
...
bool ONMainWindow::startSshd(ONMainWindow::key_types key_type)
{
    ...
    std::string key
        = "\"" + cygwinPath(etcDir + "/ssh_host_" + stringified_key_type + "_key").toStdString()
          + "\"";
    ...
    strm << clientdir << "\\sshd.exe -D -p " << clientSshPort.toInt() << " -f " << config << " -h "
         << key;
    if (debugging) {
        strm << " -E "
             << "\"" << sshLog.toStdString() << "\"";
    ...
    desktopName += qgetenv("USERNAME").toStdString();
    char* desktop = new char[desktopName.size() + 1];
    strcpy_s(desktop, desktopName.size() + 1, desktopName.c_str());

    x2goDebug << "Creating desktop: " << desktop;

    if (!CreateDesktopA(desktop, 0, 0, 0, GENERIC_ALL, 0)) {
        desktop[0] = '\0';
    ...
    QStringList arguments;
    arguments << "-f" << etcDir + "/sshd_config"
              << "-h" << etcDir + "/ssh_host_" + stringified_key_type + "_key"
              << "-D"
              << "-p" << clientSshPort;
...
void ONMainWindow::setProxyWinTitle()
{
    ...
    rez = XpmCreatePixmapFromBuffer(QX11Info::display(),
                                    proxyWinId,
                                    bytes.data(),
                                    (Pixmap*) &image,
                                    (Pixmap*) &shape,
...
void ONMainWindow::slotSetProxyWinFullscreen()
{
    ...
    Status st;
    st = XSendEvent(QX11Info::display(),
    ...
    sshConnection->executeCommand("DISPLAY=:" + resumingSession.display
                                  + " xrandr --output default --mode " + geoStr);
#endif
}

void ONMainWindow::resizeProxyWinOnDisplay(int disp)
{
    QRect geom = QGuiApplication::screens().at(disp - 1)->geometry();

    QString geoStr = "(x: " + QString("%1").arg(geom.x()) + ", y: " + QString("%1").arg(geom.y())
...
void ONMainWindow::slotConfigXinerama()
{
    ...
    QString geoStr = "(x: " + QString("%1").arg(lastDisplayGeometry.x())
    ...
    for (auto &screen: QGuiApplication::screens()) {
        ...
        if (resumingSession.fullscreen) {
            ...
            intersection = screen->geometry();
        } else
            intersection = screen->geometry().intersected(lastDisplayGeometry);
    ...
    if (xineramaScreens != newXineramaScreens) {
        ...
        foreach (QRect disp, xineramaScreens)
            screens << QString::number(disp.x()) + " " + QString::number(disp.y()) + " "
                           + QString::number(disp.width()) + " " + QString::number(disp.height());
        QString cmd = "export DISPLAY=:" + resumingSession.display + " ;printf %b '\\''"
                      + screens.join("\\n") + "'\\'' >  $HOME/.x2go/C-" + resumingSession.sessionId
                      + "/xinerama.conf";
...
void ONMainWindow::slotXineramaConfigured()
{
    ...
    XResizeWindow(QX11Info::display(),
                  proxyWinId,
                  lastDisplayGeometry.width(),
                  lastDisplayGeometry.height());
    XSync(QX11Info::display(), false);
#endif
#ifdef Q_OS_WIN
    QRect geom;
    wapiWindowRect((HWND) proxyWinId, geom);
    wapiMoveWindow((HWND) proxyWinId,
                   geom.x(),
                   geom.y(),
                   geom.width() + xinSizeInc,
                   geom.height(),
                   true);
    ...
    if (proxyWinId) {
        ...
        if (!embedMode && !(resumingSession.sessionType == x2goSession::KDRIVE)) {
            if (!useLdap) {
                ...
                if (brokerMode)
                    ...
                    st = new X2goSettings("sessions");
                xinerama
                    = st->setting()->value(sid + "/xinerama", (QVariant) defaultXinerama).toBool();
#ifndef Q_OS_WIN
        ...
        if (!startEmbedded && !(resumingSession.sessionType == x2goSession::KDRIVE)) {
            if (maximizeProxyWin) {
                ...
                wapiSetFSWindow((HWND) proxyWinId, dw.screenGeometry(this));
                /*wapiShowWindow ( ( HWND ) proxyWinId,
                   WAPI_SHOWMAXIMIZED );*/
            } else {
                wapiMoveWindow((HWND) proxyWinId, 0, 0, proxyWinWidth, proxyWinHeight, true);
            }
        }
#endif
    }
...
void ONMainWindow::slotAttachProxyWindow()
{
    ...
    if (startEmbedded) {
        ...
        act_embedContol->setIcon(QIcon(iconsPath("/32x32/detach.png")));
...
{
#ifndef Q_OS_DARWIN
    QTimer::singleShot(1000, this, SLOT(slotActivateWindow()));
...
void ONMainWindow::processCfgLine(QString line)
{
    ...
    if (lst[0] == "speed") {
        ...
        if (lst[1] == "modem")
            ...
            qCritical("%s", tr("Invalid value for argument \"speed\"").toLocal8Bit().data());
        }
        return;
    }
...
{
    passForm = new SVGFrame(images_resource_path("/svg/passform.svg"), false, bgFrame);
#ifdef Q_OS_WIN
...
#ifdef Q_OS_LINUX
    connect(login, SIGNAL(clicked()), this, SLOT(slotActivateWindow()));
    connect(pass, SIGNAL(clicked()), this, SLOT(slotActivateWindow()));
#endif
    ...
    if (defaultLayout.size() > 1) {
        ...
        connect(cbLayout,
                ...
                this,
                SLOT(slotChangeKbdLayout(QString)));
    }
}

void ONMainWindow::initStatusDlg()
{
    sessionStatusDlg = new SVGFrame(images_resource_path("/svg/passform.svg"), false, bgFrame);
    sessionStatusDlg->hide();
    if (!miniMode)
        sessionStatusDlg->setFixedSize(sessionStatusDlg->sizeHint());
    else
    ...
    slName->setText(tr("<b>Session ID:<br>Server:<br>Username:"
    ...
    connect(sbSusp, SIGNAL(clicked()), this, SLOT(slotTestSessionStatus()));
    connect(sbTerm, SIGNAL(clicked()), this, SLOT(slotTermSessFromSt()));
    connect(sbAdv, SIGNAL(clicked()), this, SLOT(slotShowAdvancedStat()));
    connect(sbExp, SIGNAL(clicked()), this, SLOT(slotExportDirectory()));
...
{
    selectSessionDlg = new SVGFrame(images_resource_path("/svg/passform.svg"), false, bgFrame);
    ...
    connect(bCancel, SIGNAL(clicked()), this, SLOT(slotCloseSelectDlg()));
    ...
    if (!brokerMode)
        ...
        model->setHeaderData(S_DISPLAY, Qt::Horizontal, QVariant((QString) tr("User")));
    }
    ...
    modelDesktop->setHeaderData(D_USER, Qt::Horizontal, QVariant((QString) tr("User")));
    modelDesktop->setHeaderData(D_DISPLAY, Qt::Horizontal, QVariant((QString) tr("Display")));
    ...
    desktopFilterCb = new QCheckBox(tr("Only my desktops"), selectSessionDlg);
    desktopFilterCb->hide();
    ...
    connect(sessTv,
            ...
            this,
            SLOT(slotResumeDoubleClick(const QModelIndex&)));
    ...
    connect(bShadow, SIGNAL(clicked()), this, SLOT(slotShadowSess()));
    connect(bShadowView, SIGNAL(clicked()), this, SLOT(slotShadowViewSess()));

    connect(desktopFilter,
            SIGNAL(textEdited(const QString&)),
            this,
            SLOT(slotDesktopFilterChanged(const QString&)));
    connect(desktopFilterCb, SIGNAL(stateChanged(int)), this, SLOT(slotDesktopFilterCb(int)));
...
void ONMainWindow::printSshDError_startupFailure()
{
    ...
    QString detailed_error_message = tr(
    ...
    QTimer::singleShot(0, [=]() {
        Non_Modal_MessageBox::critical(0l,
                                       "X2Go Client",
                                       error_message,
...
void ONMainWindow::slotPCookieReady(bool result, QString, int)
{
    /* Silence warning. */
    UNUSED(result);
}
...
void ONMainWindow::slotEmbedToolBar()
{
    ...
    if (embedTbVisible) {
        stb->clear();
        act_embedToolBar->setIcon(QIcon(iconsPath("/16x16/tbshow.png")));
        stb->addAction(act_embedToolBar);
        stb->setToolButtonStyle(Qt::ToolButtonIconOnly);
        stb->widgetForAction(act_embedToolBar)->setFixedHeight(16);
        ...
        initEmbedToolBar();
        act_embedToolBar->setIcon(QIcon(iconsPath("/32x32/tbhide.png")));
...
void ONMainWindow::slotEmbedToolBarToolTip()
{
    if (!showTbTooltip)
        return;
    QWidget* widg = stb->widgetForAction(act_embedToolBar);
...
void ONMainWindow::slotActivateWindow()
{
    if (embedMode) {
        ...
        QTimer::singleShot(50, this, SLOT(slotEmbedToolBarToolTip()));
    }
}
...
void ONMainWindow::filterDesktops(const QString& filter, bool strict)
{
    ...
    for (int row = 0; row < selectedDesktops.size(); ++row) {
        ...
        if (filter == tr("Filter") || filter.length() <= 0 || (strict && desktop[0] == filter)
            ...
            for (int j = 0; j < 2; ++j) {
                QString txt = modelDesktop->index(row, j).data().toString();
                if (sessTv->header()->sectionSize(j) < fm.horizontalAdvance(txt) + 6) {
                    sessTv->header()->resizeSection(j, fm.horizontalAdvance(txt) + 6);
                }
            }
...
{
    shadowUser = sessTv->model()->index(sessTv->currentIndex().row(), D_USER).data().toString();
    shadowDisplay = sessTv->model()->index(sessTv->currentIndex().row(), D_DISPLAY).data().toString();
    startNewSession();
}
...
long ONMainWindow::X11FindWindow(QString text, long rootWin)
{
    ...
    if (XQueryTree(QX11Info::display(), rootWin, &wRoot, &wParent, &child_list, &nChildren)) {
        for (uint i = 0; i < nChildren; ++i) {
            char* wname;
            if (XFetchName(QX11Info::display(), child_list[i], &wname)) {
                QString title(wname);
                XFree(wname);
                if (title == text) {
...
void ONMainWindow::slotInitLibssh()
{
    ...
    ssh_threads_set_callbacks(ssh_threads_get_default());
    if (ssh_init()) {
        x2goDebug << "Cannot initialize libssh.";
        QMessageBox::critical(this,
                              tr("libssh initialization failure"),
