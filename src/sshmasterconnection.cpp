...
#ifdef Q_OS_WIN
#include <QSettings>
// parse known_hosts file from libssh and export keys in registry to use with
// plink.exe
    ...
    while (!fl.atEnd()) {
        ...
        for (int i = 0; i < bytes.count();) {
            ...
            for (int j = 0; j < data.count(); ++j) {
                QString byte;
                byte.asprintf("%02x", (uchar) (data[j]));
                hex += byte;
            }
            // remove leading '0'
            for (;;) {
        ...
        x2goDebug << "Writing key to registry: "
...
SshMasterConnection::SshMasterConnection(QObject *parent,
                                         QString host,
                                         int port,
    ...
    : QThread(parent)
{
    ...
    // As we put a 512KB buffer on the stack later on, we need a bigger stack
    // space.
    ...
    x2goDebug << "SshMasterConnection, host " << host << "; port " << port << "; user " << user
              << "; useproxy " << useproxy << "; proxyserver " << proxyserver << "; proxyport "
              << proxyport;
    this->host = host;
    // If the hostname starts with "!" do not perform loginCheck() for this
    // connection
...
void SshMasterConnection::slotSshProxyConnectionOk()
{
    ...
    sshProxy->startTunnel(host,
                          port,
                          "127.0.0.1",
                          localProxyPort,
                          false,
                          this,
                          SLOT(slotSshProxyTunnelOk(int)),
                          SLOT(slotSshProxyTunnelFailed(bool, QString, int)));
}

int SshMasterConnection::copyFile(const QString &src,
                                  const QString dst,
                                  QObject *receiver,
                                  const char *slotFinished)
{
...
int SshMasterConnection::executeCommand(const QString &command,
                                        QObject *receiver,
                                        const char *slotFinished,
                                        bool overridePath)
{
...
void SshMasterConnection::addReverseTunnelConnections()
{
    ...
    for (int i = 0; i < reverseTunnelRequest.count(); ++i) {
        if (!reverseTunnelRequest[i].listen) {
            ...
            while (SSH_AGAIN == rc) {
                rc = ssh_channel_listen_forward(my_ssh_session,
                                                NULL,
                                                reverseTunnelRequest[i].forwardPort,
                                                NULL);
#else
            {
                rc = ssh_forward_listen(my_ssh_session,
                                        NULL,
                                        reverseTunnelRequest[i].forwardPort,
                                        NULL);
#endif
                ...
                if (rc == SSH_ERROR) {
                    ...
                    x2goDebug << "Forward port " << reverseTunnelRequest[i].forwardPort
                              << " failed:" << err;
...
#else
                InetPton(AF_INET, req.localHost.toLatin1(), &address.sin_addr.s_addr);
#endif

                if (::connect(sock, (struct sockaddr *) &address, sizeof(address)) != 0) {
                    QString errMsg = tr("Cannot connect to ") + req.localHost + ":"
                                     + QString::number(req.localPort);
...
int SshMasterConnection::startTunnel(const QString &forwardHost,
                                     uint forwardPort,
                                     const QString &localHost,
                                     uint localPort,
                                     bool reverse,
                                     QObject *receiver,
                                     const char *slotTunnelOk,
                                     const char *slotFinished)
{
    SshProcess *proc = new SshProcess(this, nextPid++);
    if (receiver && slotFinished) {
    ...
    if (reverse && !kerberos) {
        connect(this,
                SIGNAL(reverseTunnelOk(SshProcess *)),
                proc,
                SLOT(slotReverseTunnelOk(SshProcess *)));
        connect(this,
                SIGNAL(reverseTunnelFailed(SshProcess *, QString)),
                proc,
                SLOT(slotReverseTunnelFailed(SshProcess *, QString)));
...
void SshMasterConnection::slotSshProxyInteractionStart(SshMasterConnection *connection /* unused */,
                                                       QString prompt)
{
    Q_UNUSED(connection)
...
void SshMasterConnection::slotSshProxyInteractionUpdate(SshMasterConnection *connection /* unused */,
                                                        QString output)
{
    Q_UNUSED(connection)
...
void SshMasterConnection::slotSshProxyServerAuthError(int errCode,
                                                      QString err,
...
void SshMasterConnection::slotSshProxyTunnelFailed(bool, QString output, int)
{
    breakLoop = true;
...
void SshMasterConnection::run()
{
    ...
    if (useproxy && proxytype == PROXYSSH) {
        x2goDebug << "proxyserver: " << proxyserver << "; proxyport: " << proxyport
                  << "; proxylogin: " << proxylogin;
        sshProxy = new SshMasterConnection(0,
                                           proxyserver,
                                           proxyport,
        ...
        connect(sshProxy,
                ...
                this,
                SIGNAL(
                    needPassPhrase(SshMasterConnection *, SshMasterConnection::passphrase_types)));
        connect(sshProxy, SIGNAL(serverAuthAborted()), this, SLOT(slotSshProxyServerAuthAborted()));
        connect(sshProxy,
                SIGNAL(userAuthError(QString)),
                this,
                SLOT(slotSshProxyUserAuthError(QString)));
        connect(sshProxy,
        ...
        //         connect ( interDlg, SIGNAL(textEntered(QString)), con,
        //         SLOT(interactionTextEnter(QString))); connect ( interDlg,
        //         SIGNAL(interrupt()), con, SLOT(interactionInterruptSlot()));
    ...
    if (useproxy && proxytype == PROXYHTTP) {
        ...
        x2goDebug << "Connected to HTTP proxy server: " << proxyserver << ":" << proxyport
                  << Qt::endl;
    }
    ...
    if (state != SSH_SERVER_KNOWN_OK) {
        ...
        for (;;) {
            ...
            if (writeHostKeyReady) {
                if (writeHostKey)
                    ssh_session_update_known_hosts(my_ssh_session);
...
bool SshMasterConnection::sshConnect()
{
    ...
    if (useproxy && proxytype == PROXYSSH) {
        x2goDebug << "Connected via proxy, resetting connection values on session to "
                  << tmpBA.data() << ":" << port;
        ssh_options_set(my_ssh_session, SSH_OPTIONS_HOST, tmpBA.data());
     ...
     *
     * The tricky part is that we already set a port before (in this case our
     * proxy port.) There's no way to reset the port for this session to its
     * default value of 0 again, so we'll need to create a new session, set the
     * hostname and fetch the inferred port value from there.
     *
     * Failure to do so will trigger funny bugs like connecting to the correct
     * remote host, but at a proxied port value.
     */
        int work_port = port;

        /* Oh, yeah, and we don't really support port values of 0 for pre-0.6.0
     * libssh. Sorry. */
#if LIBSSH_VERSION_INT >= SSH_VERSION_INT(0, 6, 0)
        if (!work_port) {
            ssh_session tmp_session = ssh_new();
...
bool SshMasterConnection::userChallengeAuth()
{
    ...
    case SSH_AUTH_INFO:
        ...
        if (prompts) {
            ...
            const std::size_t challenge_auth_code_prompts_size
                = (sizeof(challenge_auth_code_prompts_) / sizeof(*challenge_auth_code_prompts_));
            ...
            if (pr.contains("challenge", Qt::CaseInsensitive)) {
                ...
                for (std::size_t i = 0; i < challenge_auth_code_prompts_size; ++i) {
                    x2goDebug << "Checking against known prompt #" << i << ": "
                              << challenge_auth_code_prompts_[i] << Qt::endl;

                    /* Ignore "garbage" at the start of the string, but require at least
           * one line to start with a known prompt. */
                    QStringList tmp_str_list = pr.split("\n", Qt::SkipEmptyParts);

                    for (QStringList::const_iterator cit = tmp_str_list.constBegin();
                         cit != tmp_str_list.constEnd();
                         ++cit) {
                    ...
                    /* Skip over other challenge auth code prompts if we found one
           * already. */
            ...
            if (has_challenge_auth_code_prompt) {
                ...
                ssh_userauth_kbdint_setanswer(my_ssh_session,
                                              0,
                                              challengeAuthVerificationCode.toLatin1());
                return userChallengeAuth();
            } else {
...
bool SshMasterConnection::userAuthWithPass()
{
    ...
    if ((!ret) && (!(method & SSH_AUTH_METHOD_PASSWORD))) {
        /* In case password auth is disabled, make sure the error message is not
     * empty. */
        QString err = ssh_get_error(my_ssh_session);
        authErrors << err;
...
bool SshMasterConnection::userAuthWithKey()
{
    ...
    /*
   * Passing an empty string as a passphrase parameter is a workaround for
   * inconsistent behavior in libssh:
   ...
   *
   * We do not want to break user experience by having libssh/OpenSSL query for
   * the passphrase on a terminal (and the client not reacting to any input
   * while this happens), so work around this inconsistency by providing an
   * empty passphrase.
   */
    ...
    /* This is TOCTU, but forced upon us by libssh's legacy function. */
    {
        ...
        if (tmp_file.open(QIODevice::ReadOnly)) {
            ...
            /* Don't pass invalid files to privatekey_from_file () - it crashes in
       * this case. */
            return (false);
        }
    }
...
#if LIBSSH_VERSION_INT >= SSH_VERSION_INT(0, 6, 0)
        rc = ssh_pki_import_privkey_file(tmp_ba.data(),
                                         tmp_ba_passphrase.data(),
                                         NULL,
                                         NULL,
                                         &priv_key);
...
bool SshMasterConnection::userAuthKrb()
{
   ...
   * is wrapped in.
   */
    QString shcmd = "bash -l -c 'echo \"X2GODATABEGIN:" + uuidStr
                    + "\"; export TERM=\"dumb\"; whoami; echo \"X2GODATAEND:" + uuidStr + "\";'";
    ...
    local_args << "-p";
#endif
    local_args << QString::number(port) << "-l" << user << host;
   ...
   */
    ...
    if (!ssh.waitForFinished(20000)) {
        sshProcErrString = ssh.errorString();
        authErrors << tr("Failed to start SSH client. Please check your "
                         "installation and GSSApi configuration.");
        authErrors << sshProcErrString;
    ...
    if (ssh.exitCode() == 0 && ssh.exitStatus() == 0 && outp == user)
        return true;
    authErrors << tr("Check your GSSApi configuration or choose another "
        ...
        while (ssh_channel_is_open(channel) && !ssh_channel_is_eof(channel)) {
            ...
            if (textToSend.length() > 0) {
                ...
                ssh_channel_write(channel,
                                  textToSend.toLocal8Bit().constData(),
                                  textToSend.length());
            }
            if (interrupt) {
                break;
...
void SshMasterConnection::addChannelConnection(SshProcess *creator,
                                               int sock,
                                               QString forwardHost,
                                               int forwardPort,
                                               QString localHost,
...
void SshMasterConnection::copy()
{
    for (int i = copyRequests.size() - 1; i >= 0; --i) {
        ...
        x2goDebug << "SSH Master Connection copy - dst path:" << dstPath << " file:" << dstFile
                  << Qt::endl;
        ssh_scp scp = ssh_scp_new(my_ssh_session,
                                  SSH_SCP_WRITE | SSH_SCP_RECURSIVE,
                                  dstPath.toLatin1());
        if (scp == NULL) {
            x2goDebug << "Error allocating SCP session: " << ssh_get_error(my_ssh_session)
                      << Qt::endl;
            return;
        }
        int rc = ssh_scp_init(scp);
        if (rc != SSH_OK) {
            x2goDebug << "Error initializing SCP session: " << ssh_get_error(my_ssh_session)
                      << Qt::endl;
            ssh_scp_free(scp);
            return;
        }
        ...
        x2goDebug << "scp ok: " << copyRequests[i].src << " -> " << user << "@" << host << ":"
                  << copyRequests[i].dst << Qt::endl;
        copyRequests.removeAt(i);
        ssh_scp_close(scp);
        ssh_scp_free(scp);
    }
...
void SshMasterConnection::channelLoop()
{
    forever {
        ...
        for (int i = channelConnections.size() - 1; i >= 0; --i) {
            ...
            if (rez == SSH_EOF) {
                x2goDebug << "EOF on channel " << channel
                          << "; SshProcess object: " << channelConnections[i].creator->pid;
                ...
                if (nbytes > 0) {
                    if (tcpSocket > 0) {
                        if (send(tcpSocket, buffer, nbytes, 0) != nbytes) {
                            ...
                            x2goDebug << "Error writing " << nbytes << " to TCP socket" << tcpSocket
                                      << Qt::endl;
                            emit ioErr(channelConnections[i].creator, errMsg, "");
                            finalize(i);
                            continue;
                        }
                        //                      x2goDebug<<"wrote "<<nbytes<<" to tcp socket
                        //                      "<<tcpSocket<<Qt::endl;
                    } else {
            ...
            if (FD_ISSET(tcpSocket, &rfds)) {
                ...
                //                  x2goDebug<<nbytes<<" bytes from tcp socket
                //                  "<<tcpSocket<<Qt::endl;
                if (nbytes > 0) {
                    ...
                    //                      x2goDebug<<nbytes<<" bytes wrote to
                    //                      channel"<<channel<<Qt::endl;
                }
                if (nbytes < 0) {
                    //////ERROR!!!!!////////
                    QString err = "";
 ...
 */
bool SshMasterConnection::createChannelConnection(int i,
                                                  int &maxsock,
                                                  fd_set &rfds,
                                                  ssh_channel *read_chan)
{
    int tcpSocket = channelConnections.at(i).sock;
    if (tcpSocket > 0)
    ...
    if (channelConnections.at(i).channel == 0l) {
        ...
        if (tcpSocket > 0) {
            x2goDebug << "Forwarding parameters: from remote ("
                      << channelConnections.at(i).forwardHost << ":"
                      << channelConnections.at(i).forwardPort << ") to local ("
                      << channelConnections.at(i).localHost << ":"
                      << channelConnections.at(i).localPort << ")";
       ...
       */
            ...
            if (!tmp_session) {
                ...
                /* The host and port might be a shorthand and zero, so fetch the actual
         * data. */
                if (ssh_options_parse_config(tmp_session, NULL) < 0) {
                    x2goDebug << "Warning: unable to parse the SSH config file.";
                }
            ...
            if (ssh_channel_open_session(channel) != SSH_OK) {
                ...
                return (false);
            } else if (ssh_channel_request_exec(channel, channelConnections[i].command.toLatin1())
                       != SSH_OK) {
                QString err = ssh_get_error(my_ssh_session);
                QString errorMsg = tr("%1 failed.").arg("ssh_channel_request_exec");
