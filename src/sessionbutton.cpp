    ...
    : SVGFrame(mw->images_resource_path("/svg/sessionbut.svg"), false, parent)
{
    ...
    connect(sessMenu, SIGNAL(aboutToHide()), this, SLOT(slotMenuHide()));

    act_edit = sessMenu->addAction(QIcon(mw->iconsPath("/16x16/edit.png")),
                                   tr("Session preferences ..."));
#if (!defined Q_WS_HILDON) && (!defined Q_OS_DARWIN)
    act_createIcon = sessMenu->addAction(QIcon(mw->iconsPath("/16x16/create_file.png")),
                                         tr("Create session icon on desktop ..."));
#endif
    act_remove = sessMenu->addAction(QIcon(mw->iconsPath("/16x16/delete.png")),
                                     tr("Delete session"));

    connect(act_edit, SIGNAL(triggered(bool)), this, SLOT(slotEdit()));

    connect(act_remove, SIGNAL(triggered(bool)), this, SLOT(slotRemove()));
#if (!defined Q_WS_HILDON) && (!defined Q_OS_DARWIN)
    connect(act_createIcon, SIGNAL(triggered(bool)), this, SLOT(slotCreateSessionIcon()));
#endif
...
void SessionButton::redraw()
{
    ...
    QString name = st->setting()->value(sid + "/name", (QVariant) tr("New Session")).toString();
    ...
    sessStatus->setText("");
    QString status = st->setting()->value(sid + "/status", (QVariant) QString()).toString();
    if (status == "R") {
    ...
    int suspended = st->setting()->value(sid + "/suspended", (QVariant) QString()).toUInt();

    int running = st->setting()->value(sid + "/running", (QVariant) QString()).toUInt();
    ...
    QString sessIcon = wrap_legacy_resource_URIs(
        st->setting()
            ->value(sid + "/icon", (QVariant) par->iconsPath("/128x128/x2gosession.png"))
            .toString());
    ...
    //     x2goDebug << "Creating QPixmap with session icon: " <<
    //     sessIcon.toLatin1 () << ".";
    ...
    delete pix;
    QString sv = st->setting()->value(sid + "/host", (QVariant) QString()).toString();
    QString uname = st->setting()->value(sid + "/user", (QVariant) QString()).toString();
    server->setText(uname + "@" + sv);

    QString command = st->setting()->value(sid + "/command", (QVariant) tr("KDE")).toString();
    rootless = st->setting()->value(sid + "/rootless", false).toBool();
    published = st->setting()->value(sid + "/published", false).toBool();
...
#ifdef Q_OS_LINUX
        if (st->setting()->value(sid + "/directrdp", (QVariant) false).toBool())
            directRDP = true;
#endif
...
#ifdef Q_OS_LINUX
        if (st->setting()->value(sid + "/directxdmcp", (QVariant) false).toBool()) {
            directRDP = true;
    ...
    if (!directRDP) {
        ...
        for (auto &screen: QGuiApplication::screens()) {
            geomBox->addItem(tr("Display ") + QString::number(i));

            // add maximun available area
            geomBox->addItem(
                QString::number(screen->availableGeometry().width()) + "x"
                + QString::number(screen->availableGeometry().height()));
            i++;
        }
    }
#ifndef Q_WS_HILDON
...
#endif
    if (st->setting()->value(sid + "/fullscreen", (QVariant) false).toBool()) {
        geom->setText(tr("fullscreen"));
    } else if (st->setting()->value(sid + "/multidisp", (QVariant) false).toBool() && !directRDP) {
        uint disp = st->setting()->value(sid + "/display", (QVariant) 1).toUInt();
        if (disp <= (uint) QGuiApplication::screens().size()) {
...
#ifndef Q_WS_HILDON
        QString g = QString::number(st->setting()->value(sid + "/width").toInt());
        g += "x" + QString::number(st->setting()->value(sid + "/height").toInt());
        geom->setText(g);
    ...
    sound->setFixedSize(fm.size(Qt::TextSingleLine, sound->text()) + QSize(8, 4));
...
void SessionButton::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (!editable)
        return;
    if (cmd->isVisible())
        if (event->x() > cmd->x() && event->x() < cmd->x() + cmd->width() && event->y() > cmd->y()
            && event->y() < cmd->y() + cmd->height()) {
...
{
    sessMenu->popup(
        mapToGlobal(QPoint(editBut->x() + editBut->width(), editBut->y() + editBut->height())));
}
