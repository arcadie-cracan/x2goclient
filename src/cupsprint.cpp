...
QString CUPSPrint::getDefaultUserPrinter()
{
    ...
    QString defPrint = st.setting()->value("CUPS/defaultprinter", "").toString();
    if (defPrint.length() > 0) {
        cups_dest_t *dest = cupsGetDest(defPrint.toLatin1(), 0l, num_dests, dests);
        if (dest)
            return defPrint;
    }
...
bool CUPSPrint::getPrinterInfo(const QString &printerName,
                               QString &info,
                               bool &acceptJobs,
                               QString &location,
                               QString &model,
                               ...
                               QString &stateReason)
{
    cups_dest_t *dest = cupsGetDest(printerName.toLatin1(), 0l, num_dests, dests);
    if (!dest)
        return false;
    ...
    info = QString::fromLocal8Bit(cupsGetOption("printer-info", dest->num_options, dest->options));
    ...
    QString st = cupsGetOption("printer-state", dest->num_options, dest->options);
    state = NDEF;
    if (st == "3")
        state = IDLE;
    if (st == "4")
    ...
    stateReason = QString::fromLocal8Bit(
        cupsGetOption("printer-state-reasons", dest->num_options, dest->options));
    return true;
}
...
bool CUPSPrint::getOptionValue(const QString &option, QString &value, QString &valueText)
{
    if (!ppd)
        return false;
...
int CUPSPrint::getOptionValues(const QString &option, QStringList &values, QStringList &descriptions)
{
    values.clear();
    descriptions.clear();
    if (!ppd)
...
int CUPSPrint::getOptionsList(const QString &groupName, QStringList &names, QStringList &texts)
{
    names.clear();
    texts.clear();
    if (!ppd)
    ...
    for (int i = 0; i < ppd->num_groups; ++i) {
        ppd_group_t *group = &(ppd->groups[i]);
        if (groupName.length() > 0 && groupName != QString::fromLocal8Bit(group->name))
            continue;
...
bool CUPSPrint::setValue(const QString &option,
                         const QString &value,
                         QString &conflict_opt,
                         QString &conflict_val)
{
    if (!ppd)
        return false;
    ...
    for (int i = 0; i < ppd->num_consts; ++i) {
        ...
        if (option == ppd->consts[i].option1 && value == ppd->consts[i].choice1) {
            confOpt = ppd->consts[i].option2;
            confVal = ppd->consts[i].choice2;
        } else if (option == ppd->consts[i].option2 && value == ppd->consts[i].choice2) {
...
void CUPSPrint::saveOptions()
{
    ...
    st.setting()->setValue("CUPS/options/" + currentPrinter, QVariant(options));
}

void CUPSPrint::loadUserOptions()
{
    X2goSettings st("printing");
    QStringList options = st.setting()->value("CUPS/options/" + currentPrinter).toStringList();
    ...
    for (int i = 0; i < ppd->num_groups; ++i) {
        ...
        for (int j = 0; j < group->num_options; ++j) {
            ...
            if (!getOptionValue(option->keyword, val, valtext))
                continue; // something is wrong here
            if (val != option->defchoice) {
                num_options = cupsAddOption(option->keyword, val.toLatin1(), num_options, &options);
            }
        }
    }
    cupsPrintFile(currentPrinter.toLatin1(), file.toLatin1(), title.toLatin1(), num_options, options);
