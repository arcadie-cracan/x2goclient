...
HWND wapiSetParent(HWND child, HWND par)
{
    ...
    if (par)
        SetWindowLong(child, GWL_STYLE, GetWindowLong(child, GWL_STYLE) | WS_CHILD);
    else
        SetWindowLong(child, GWL_STYLE, GetWindowLong(child, GWL_STYLE) & ~WS_CHILD);
    SetWindowLong(child, GWL_STYLE, GetWindowLong(child, GWL_STYLE) | WS_POPUP);
    return wn;
}
...
bool wapiClientRect(HWND wnd, QRect &rect)
{
    ...
    if (GetClientRect(wnd, &rcWindow)) {
        rect.setCoords(rcWindow.left, rcWindow.top, rcWindow.right, rcWindow.bottom);
        return true;
    }
    return false;
}
...
bool wapiWindowRectWithoutDecoration(HWND wnd, QRect &rect)
{
    ...
    if (GetClientRect(wnd, &rcWindow)) {
        ...
        rect.setRect(pnt.x, pnt.y, rcWindow.right - rcWindow.left, rcWindow.bottom - rcWindow.top);
        return true;
    }
    return false;
}
...
bool wapiMoveWindow(HWND wnd, int x, int y, int width, int height, bool repaint)
{
...
void wapiSetWindowIcon(HWND wnd, const QPixmap &icon)
{
    ...
    largeIcon = QtWin::toHICON(
        icon.scaled(iconx, icony, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
    smallIcon = QtWin::toHICON(
        icon.scaled(smallx, smally, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
...
QString wapiShortFileName(const QString &longName)
{
    ...
    length = GetShortPathName((LPCTSTR) longName.utf16(), buffer, length);
    if (!length) {
        delete[] buffer;
        return longName;
    }
...
QString getNameFromSid(PSID psid, QString *systemName)
{
    ...
    LookupAccountSid(0, psid, name, &length, sysName, &dlength, &eUse);
    if (!length) {
        return QString();
    }
    ...
    if (!LookupAccountSid(0, psid, name, &length, sysName, &dlength, &eUse)) {
        delete[] name;
        delete[] sysName;
        return QString();
    }

    QString strName = QString::fromUtf16((const ushort *) name);
    if (systemName)
        *systemName = QString::fromUtf16((const ushort *) sysName);
    delete[] sysName;
    delete[] name;
    return strName;
}
...
QString getStringFromSid(PSID psid)
{
    ...
    QString str = QString::fromUtf16((const ushort *) stringSid);
    LocalFree(stringSid);
    return str;
}
                     ...
                     QString *retSysName)
{
    ...
    if (primaryGroupSID || primaryGroupName) {
        ...
        DWORD dwResult = 0;
        DWORD dwSize = 0;
        if (!GetTokenInformation(hToken, TokenPrimaryGroup, NULL, dwSize, &dwSize)) {
        ...
        pGroupInfo = (PTOKEN_PRIMARY_GROUP) GlobalAlloc(GPTR, dwSize);

        if (!GetTokenInformation(hToken, TokenPrimaryGroup, pGroupInfo, dwSize, &dwSize)) {
            if (pGroupInfo)
        ...
        if (primaryGroupSID) {
            *primaryGroupSID = getStringFromSid(pGroupInfo->PrimaryGroup);
        }
        if (primaryGroupName) {
            *primaryGroupName = getNameFromSid(pGroupInfo->PrimaryGroup, retSysName);
        }
    ...
    if (retSid || retUname) {
        ...
        if (retSid) {
            *retSid = getStringFromSid(pUserInfo->User.Sid);
        }
        if (retUname) {
            *retUname = getNameFromSid(pUserInfo->User.Sid, retSysName);
        }
        if (pUserInfo)
            GlobalFree(pUserInfo);
    }
...
void wapiShellExecute(const QString &operation,
                      const QString &file,
...
QStringList wapiGetLocalPrinters()
{
    ...
    DWORD bufSize = 0;
    DWORD sizeNeeded = 0;
    EnumPrinters(PRINTER_ENUM_LOCAL, 0, 4, NULL, bufSize, &sizeNeeded, &sizeOfArray);
    if (!sizeNeeded) {
        return printers;
    }
    ...
    if (!info_array)
        return printers;
    bufSize = sizeNeeded;
    EnumPrinters(PRINTER_ENUM_LOCAL, 0, 4, (LPBYTE) info_array, bufSize, &sizeNeeded, &sizeOfArray);
    if (!sizeNeeded || !sizeOfArray) {
        delete[] info_array;
        return printers;
    }
    for (uint i = 0; i < sizeOfArray; ++i) {
        printers << QString::fromUtf16((const ushort *) (info_array[i].pPrinterName));
    }
    delete[] info_array;
    return printers;
}
