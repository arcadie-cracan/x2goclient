 ...
 *   Copyright (C) 2016-2020 by Mihai Moldovan <ionic@ionic.de>            *
 *                                                                         *
...
QString convert_to_rich_text(const QString &text, bool force)
{
    ...
    if (force) {
        // This is a workaround for a bug in Qt. Even though we set Qt::RichText as
        // the text format later on, the informative text is not recognized as rich
        // text, UNLESS a HTML tag is used ON THE VERY FIRST LINE. Make sure, that
        // there always is one...
...
QString git_changelog_extract_commit_sha(const QString &gitlog)
{
   ...
   * We know that a newline character should be somewhere at the beginning of
   * the string. We don't need to have Qt split the string up completely as we
   * only care about a substring: from start to the first newline character.
   */
    ...
    if (0 < pos) {
        ...
        if (0 < pos) {
            ...
            if ((0 < pos_paren_start) && (pos_paren_start < pos)) {
                ...
                // Either starting parenthesis not found or starting parenthesis comes
                // first.
                ret = "";
            }
        } else {
...
void show_XQuartz_generic_error(const QString &main_error, const QString &additional_info)
{
    show_RichText_WarningMsgBox(
        main_error,
        additional_info
            + QObject::tr("MacPorts users, please install either the port "
                          "<b>xorg-server</b>\n"
                          "or the port <b>xorg-server-devel</b>.\n"
                          "Upon successful installation, please follow the instructions "
                          "printed\n"
   ...
   */
    ...
    for (int i = 0; i < orig_path_list.length(); ++i) {
        for (int y = 0; y < clean_add.size(); ++y) {
            if (!found[y]) {
                if ((orig_path_list[i] == QString(clean_add[y]))
                    || (orig_path_list[i] == QString(clean_add[y] + '/'))) {
                    found[y] = true;
                    break;
                }
            }
    ...
    for (std::size_t i = 0; i < found.size(); ++i) {
        x2goDebug << "found entry i (" << i << ") in orig_path_list (" << clean_add[i]
                  << "): " << found[i];
    }
...
QString find_binary(const QString &path, const QString &binary_name)
{
    ...
    if (!(binary_name.isEmpty())) {
        ...
        for (QStringList::const_iterator const_it = path_list.constBegin();
             const_it != path_list.constEnd();
             ++const_it) {
            cur_path = *const_it;
            ...
            if ((tmp_file_info.exists()) && (tmp_file_info.isExecutable())) {
                ...
                x2goDebug << "Binary at " << cur_path << " either does not exist ("
                          << !(tmp_file_info.exists()) << ") or is not executable ("
                          << tmp_file_info.isExecutable() << ")";
            }
        }
    }
