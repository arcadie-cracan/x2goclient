...
CUPSPrinterSettingsDialog::CUPSPrinterSettingsDialog(QString prnName,
    ...
    : QDialog(parent, flags)
{
    ...
    setGeneralTab();
    setPPDTab();
    connect((QObject *) (ui.buttonBox->button(QDialogButtonBox::RestoreDefaults)),
            SIGNAL(clicked()),
            this,
            SLOT(slot_restoreDefaults()));
    connect((QObject *) (ui.buttonBox->button(QDialogButtonBox::Save)),
            SIGNAL(clicked()),
            this,
            SLOT(slot_saveOptions()));
    connect((QObject *) (ui.buttonBox->button(QDialogButtonBox::Cancel)),
            SIGNAL(clicked()),
            this,
            SLOT(reject()));
    connect((QObject *) (ui.buttonBox->button(QDialogButtonBox::Ok)),
            SIGNAL(clicked()),
            this,
            SLOT(slot_ok()));
}
...
void CUPSPrinterSettingsDialog::setCbBox(QComboBox *cb, QString optionKeyword)
{
    QStringList values;
    QStringList descriptions;
    int cur_val = m_cups->getOptionValues(optionKeyword, values, descriptions);
    if (cur_val == -1)
        cb->setEnabled(false);
    else {
...
void CUPSPrinterSettingsDialog::slot_optionSelected(QTreeWidgetItem *current, QTreeWidgetItem *)
{
    ...
    if (current)
        if (current->childCount() == 0) {
            ...
            int selectedValue = m_cups->getOptionValues(current->text(2), valueNames, valueTexts);
            for (int i = 0; i < valueNames.size(); ++i) {
                QTreeWidgetItem *ritem = new QTreeWidgetItem(ui.optionsTree, QTreeWidgetItem::Type);
                ritem->setText(0, valueTexts[i]);
                ritem->setText(1, valueNames[i]);
                if (i == selectedValue)
                    ui.optionsTree->setCurrentItem(ritem);
            }
            return;
        }
...
void CUPSPrinterSettingsDialog::slot_valueSelected(QTreeWidgetItem *current, QTreeWidgetItem *)
{
    if (!current)
        return;
...
{
    disconnect(ui.cbPageSize, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changePSize(int)));
    disconnect(ui.cbMediaType, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changePType(int)));
    disconnect(ui.cbInputSlot, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changeISlot(int)));
    disconnect(ui.rbNone, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
    disconnect(ui.rbShort, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
    disconnect(ui.rbLong, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
    ui.cbPageSize->clear();
    ...
    connect(ui.cbPageSize, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changePSize(int)));
    connect(ui.cbMediaType, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changePType(int)));
    connect(ui.cbInputSlot, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_changeISlot(int)));
    connect(ui.rbNone, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
    connect(ui.rbShort, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
    connect(ui.rbLong, SIGNAL(clicked()), this, SLOT(slot_changeDuplex()));
}
...
void CUPSPrinterSettingsDialog::setPPDTab()
{
    disconnect(ui.ppdTree,
               ...
               this,
               SLOT(slot_optionSelected(QTreeWidgetItem *, QTreeWidgetItem *)));

    disconnect(ui.optionsTree,
               SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)),
               this,
               SLOT(slot_valueSelected(QTreeWidgetItem *, QTreeWidgetItem *)));
    ...
    QString stateReason;
    QString valueName, valueText;
    UNUSED(m_cups->getPrinterInfo(printer, info, acceptJobs, location, model, state, stateReason));

    ui.ppdTree->clear();
    QTreeWidgetItem *ritem = new QTreeWidgetItem((QTreeWidgetItem *) 0, QTreeWidgetItem::Type);
    ...
    for (int i = 0; i < grName.size(); ++i) {
        QTreeWidgetItem *gritem = new QTreeWidgetItem(ritem, QTreeWidgetItem::Type);
        ...
        for (int j = 0; j < optName.size(); ++j) {
            QTreeWidgetItem *optitem = new QTreeWidgetItem(gritem, QTreeWidgetItem::Type);
            optitem->setText(0, optText[j]);
            optitem->setText(2, optName[j]);
            m_cups->getOptionValue(optName[j], valueName, valueText);
...
bool CUPSPrinterSettingsDialog::setNewValue(const QString &option, const QString &value)
{
    QString confVal, confOpt;
    bool res = m_cups->setValue(option, value, confOpt, confVal);
    if (!res) {
        QString textMessage = tr("This value is in conflict with another option.");
        QString txt;
        ...
        QMessageBox::critical(this, tr("Options conflict."), textMessage);
    }
    return res;
}
...
void CUPSPrinterSettingsDialog::changeGeneralOption(const QString &option, const QString &val)
{
