     ...
     */
        ...
        if (0 == tmp_pid) {
            ...
            if (0 != err) {
                ...
                std::cerr << "Continuing with normal operation, but process might kill "
                             "itself before tree vanishes."
                          << std::Qt::endl;
            }
...
void real_kill_pgroup(const pid_t pgid)
{
    ...
    if (0 != killpg(pgid, SIGTERM)) {
        ...
        std::cerr << "WARNING: unable to send SIGTERM to process group '" << pgid
                  << "': " << std::strerror(saved_errno) << std::Qt::endl;
    }
...
int unix_cleanup(const pid_t parent)
{
   ...
   */
    ...
    if (0 != sigemptyset(&empty_set)) {
        const int saved_errno = errno;
        std::cerr << "Unable to fetch empty signal set: " << std::strerror(saved_errno)
                  << std::Qt::endl;
        kill_pgroup(-1);
    ...
    if (0 != sigprocmask(SIG_SETMASK, &empty_set, NULL)) {
        const int saved_errno = errno;
        std::cerr << "Unable to set empty signal set: " << std::strerror(saved_errno)
                  << std::Qt::endl;
        kill_pgroup(-1);
