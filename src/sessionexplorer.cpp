...
SessionExplorer::SessionExplorer(ONMainWindow *p)
    : QObject(p)
{
    parent = p;
    lastSession = 0;
...
void SessionExplorer::resize()
{
    ...
    QString displayText = metrics.elidedText(" " + currentPath,
                                             Qt::ElideLeft,
                                             pathLabel->width() - 6);
...
void SessionExplorer::getFoldersFromConfig()
{
    ...
    foreach (folder, folders) {
        if (folder.indexOf("icon_") == 0) {
            folder = folder.mid(static_cast<int>(strlen("icon_")));
            folder.replace("::", "/");
...
void SessionExplorer::slotCreateDesktopIcon(SessionButton *bt)
{
    ...
    QString name = st.setting()->value(bt->id() + "/name", (QVariant) tr("New Session")).toString();

    // PyHoca-GUI uses the slash as separator for cascaded menus, so let's handle
    // these on the file system
    name.replace("/", "::");

    QString sessIcon = wrap_legacy_resource_URIs(
        st.setting()
            ->value(bt->id() + "/icon", (QVariant) parent->iconsPath("/128x128/x2gosession.png"))
            .toString());
    ...
    QFile file(
        ...
        QDesktopServices::storageLocation(QDesktopServices::DesktopLocation) + "/" + name
        + ".desktop");
#else
...
SessionButton *SessionExplorer::createBut(const QString &id)
{
    ...
    sessions.append(l);
    connect(l, SIGNAL(signal_edit(SessionButton *)), this, SLOT(slotEdit(SessionButton *)));

    connect(l,
            SIGNAL(signal_remove(SessionButton *)),
            this,
            SLOT(slotDeleteButton(SessionButton *)));

    connect(l,
            SIGNAL(sessionSelected(SessionButton *)),
            parent,
...
void SessionExplorer::placeButtons()
{
    ...
    for (int i = 0; i < folders.size(); ++i) {
        if ((folders[i]->getPath() != currentPath)
            || (getFolderChildren(folders[i]).length() <= 0)) {
            folders[i]->hide();
            continue;
        }
    ...
    for (int i = 0; i < sessions.size(); ++i) {
        ...
        int horizontalPosition = (parent->getMiniMode())
                                     ? (parent->getUsersArea()->width() - 260) / 2
                                     : (parent->getUsersArea()->width() - 360) / 2;

        sessions[i]->move(horizontalPosition, currentVerticalPosition + 5);
        ...
        //         x2goDebug<<"last
        //         session"<<(void*)lastSession<<"current"<<(void*)sessions[i];
        sessions[i]->show();
    ...
    if (currentVerticalPosition) {
        parent->getUsersFrame()->setFixedHeight(currentVerticalPosition);
    }
...
QStringList SessionExplorer::getFolderChildren(FolderButton *folder)
{
    ...
    QString normPath
        = (folder->getPath() + "/" + folder->getName()).split("/", Qt::SkipEmptyParts).join("/");
...
void SessionExplorer::createFolder(QString path)
{
    ...
    FolderButton *fb = new FolderButton(parent,
                                        parent->getUsersFrame(),
                                        currentPath.join("/"),
                                        tails.last());
    connect(fb,
            SIGNAL(folderSelected(FolderButton *)),
            this,
            SLOT(slotFolderSelected(FolderButton *)));
    folders << fb;
}
...
{
    for (int i = 0; i < folders.count(); ++i) {
        QString normPath = (folders[i]->getPath() + "/" + folders[i]->getName())
...
void SessionExplorer::setFolderIcon(QString path, QString icon)
{
    ...
    if (!pix.isNull()) {
        ...
        FolderButton *b;
        foreach (b, folders) {
            if ((b->getPath() + "/" + b->getName()).split("/", Qt::SkipEmptyParts).join("::")
                == path) {
                b->loadIcon();
                break;
            }
