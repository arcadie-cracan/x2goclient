...
void PrintProcess::slot_processFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    disconnect(this,
               SIGNAL(finished(int, QProcess::ExitStatus)),
               this,
               SLOT(slot_processFinished(int, QProcess::ExitStatus)));
    QFile::remove(pdfFile);
    if (exitCode == 0 && exitStatus == QProcess::NormalExit) {
        if (!printStdIn) {
            if (!QProcess::startDetached(printCmd, (QStringList() << "\"" + psFile + "\"")))
            ...
            proc->start(printCmd, QStringList());
        }
    } else
...
#else
    winX2goPrinter = st.setting()->value("print/defaultprinter", wapiGetDefaultPrinter()).toString();
#endif
#ifdef Q_WS_HILDON
...
void PrintProcess::print()
{
...
#endif // Q_WS_HILDON
    {
        if (!printPs) {
            if (!printStdIn) {
                if (!QProcess::startDetached(printCmd,  (QStringList() << "\"" + pdfFile + "\"")))
                    slot_error(QProcess::FailedToStart);
            ...
            connect(proc,
                    SIGNAL(error(QProcess::ProcessError)),
                    this,
                    SLOT(slot_pdf2psError(QProcess::ProcessError)));
#ifndef Q_OS_WIN
            ...
            env.replaceInStrings(QRegExp("^PATH=(.*)", Qt::CaseInsensitive), "PATH=\\1;" + wdir);
...
void PrintProcess::slot_error(QProcess::ProcessError)
{
    ...
    QMessageBox::critical(0l, tr("Printing error."), message);
}
