    ...
    : QWidget(parent)
{
    ...
    connect(ui.cbPrinters, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_printerSelected(int)));

    connect(ui.pbProps, SIGNAL(clicked()), this, SLOT(slot_printerSettings()));
}
...
void CUPSPrintWidget::slot_printerSelected(int index)
{
    ...
    QString stateReason;
    if (!m_cups->getPrinterInfo(
            ui.cbPrinters->currentText(), info, acceptJobs, location, model, state, stateReason))
        return;
    QString stText;
    switch (state) {
    case CUPSPrint::IDLE:
    ...
    (acceptJobs) ? ui.lJAccept->setText(tr("Yes")) : ui.lJAccept->setText(tr("No"));
...
{
    CUPSPrinterSettingsDialog dlg(ui.cbPrinters->currentText(), m_cups, this);
    dlg.exec();
}
