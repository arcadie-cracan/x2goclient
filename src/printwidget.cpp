    ...
    : QWidget(parent)
{
    ...
    connect(ui.cbPrintCmd, SIGNAL(toggled(bool)), pwid, SLOT(setDisabled(bool)));
#else
#ifdef Q_OS_WIN
    connect(ui.cbPrintCmd, SIGNAL(toggled(bool)), ui.cbWinPrinter, SLOT(setDisabled(bool)));
    connect(ui.cbPrintCmd, SIGNAL(toggled(bool)), ui.lWinPrinter, SLOT(setDisabled(bool)));
    connect(ui.cbPrintCmd, SIGNAL(toggled(bool)), ui.lWinInfo, SLOT(setDisabled(bool)));
    ...
    QLabel *rtfm = new QLabel(tr("Please configure your client side printing settings.<br><br>"
...
#endif
#endif
    connect(ui.pbPrintCmd, SIGNAL(clicked()), this, SLOT(slot_editPrintCmd()));

    QButtonGroup *bg = new QButtonGroup();
    bg->addButton(ui.rbPrint);
    bg->addButton(ui.rbView);
    loadSettings();
    connect(ui.cbShowDialog, SIGNAL(toggled(bool)), this, SIGNAL(dialogShowEnabled(bool)));
#if (defined Q_OS_WIN)
    ui.label->hide();
    ui.leOpenCmd->hide();
#endif
...
void PrintWidget::loadSettings()
{
    ...
    QString prcmd = st.setting()->value("print/command", "").toString();
#ifdef Q_OS_WIN
    defaultPrinter = st.setting()->value("print/defaultprinter", defaultPrinter).toString();
...
#endif
    ui.cbShowDialog->setChecked(st.setting()->value("showdialog", true).toBool());
    ...
    ui.cbPrintCmd->setChecked(st.setting()->value("print/startcmd", false).toBool());
#ifndef Q_OS_WIN
    if (prcmd == "")
        prcmd = "lpr";
#endif
    ...
    ui.leOpenCmd->setText(st.setting()->value("view/command", "xpdf").toString());
}
...
void PrintWidget::saveSettings()
{
    ...
    st.setting()->setValue("showdialog", QVariant(ui.cbShowDialog->isChecked()));
    st.setting()->setValue("pdfview", QVariant(ui.rbView->isChecked()));
    st.setting()->setValue("print/startcmd", QVariant(ui.cbPrintCmd->isChecked()));
    st.setting()->setValue("print/command", QVariant(ui.lePrintCmd->text()));
    st.setting()->setValue("print/stdin", QVariant(printStdIn));
    st.setting()->setValue("print/ps", QVariant(printPs));

    st.setting()->setValue("view/open", QVariant(ui.rbOpen->isChecked()));
    st.setting()->setValue("view/command", QVariant(ui.leOpenCmd->text()));
#ifdef Q_OS_WIN
    st.setting()->setValue("print/defaultprinter", QVariant(ui.cbWinPrinter->currentText()));
#endif
#if (!defined Q_OS_WIN) && (!defined Q_WS_HILDON)
    pwid->savePrinter();
#endif
...
bool PrintWidget::gsInfo(QString &version, QString &pdf2ps)
{
    ...
    for (int i = 0; i < keys.size(); ++i) {
        if (keys[i].indexOf("GS_LIB") != -1) {
            ...
            libs = libs.split(";")[0];
            if (QFile::exists(libs + "\\pdf2ps.bat") && v.toFloat() > version.toFloat()) {
                version = v;
...
bool PrintWidget::gsViewInfo(QString &version, QString &gsprint)
{
    ...
    for (int i = 0; i < keys.size(); ++i) {
        QString v = keys[i];
        QString libs = st.value(keys[i], "").toString();
        if (QFile::exists(libs + "\\gsview\\gsprint.exe") && v.toFloat() > version.toFloat()) {
            version = v;
