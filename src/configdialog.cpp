    ...
    : QDialog(parent, f)
{
    ...
    cbMinimizeTray = new QCheckBox(tr("Hide to system tray after connection is established"),
                                   frTray);
    cbMaxmizeTray = new QCheckBox(tr("Restore from system tray after session is disconnected"),
                                  frTray);
    ...
    if (!embedMode) {
        ...
        laiLay->addWidget(new QLabel(tr("Failover server 1 URL:"), frLdap));
        laiLay->addWidget(new QLabel(tr("Failover server 2 URL:"), frLdap));
        ...
        gbLDAP->setChecked(
            st.setting()->value("LDAP/useldap", (QVariant) par->retUseLdap()).toBool());
        ldapServer->setText(
            st.setting()->value("LDAP/server", (QVariant) par->retLdapServer()).toString());
        port->setValue(st.setting()->value("LDAP/port", (QVariant) par->retLdapPort()).toInt());
        ldapServer1->setText(
            st.setting()->value("LDAP/server1", (QVariant) par->retLdapServer1()).toString());
        port1->setValue(st.setting()->value("LDAP/port1", (QVariant) par->retLdapPort1()).toInt());
        ldapServer2->setText(
            st.setting()->value("LDAP/server2", (QVariant) par->retLdapServer2()).toString());
        port2->setValue(st.setting()->value("LDAP/port2", (QVariant) par->retLdapPort2()).toInt());
        ldapBase->setText(
            st.setting()->value("LDAP/basedn", (QVariant) par->retLdapDn()).toString());
        frLdap->setEnabled(gbLDAP->isChecked());
        frLay->addWidget(gbLDAP);
        connect(gbLDAP, SIGNAL(toggled(bool)), frLdap, SLOT(setEnabled(bool)));

        connect(gbLDAP, SIGNAL(toggled(bool)), this, SLOT(slot_checkOkStat()));
        connect(ldapBase, SIGNAL(textChanged(const QString &)), this, SLOT(slot_checkOkStat()));
        connect(ldapServer, SIGNAL(textChanged(const QString &)), this, SLOT(slot_checkOkStat()));
    }
#endif // USELDAP
    ...
    pbOpenExec = new QPushButton(QIcon(((ONMainWindow *) parent)->iconsPath("/32x32/file-open.png")),
                                 QString(),
                                 xgb);
    ...
    if (embedMode) {
        cbStartEmbed = new QCheckBox(tr("Start session embedded inside website"), fr);
        frLay->addWidget(cbStartEmbed);
        advancedOptions = new QPushButton(tr("Advanced options") + " >>", this);
        connect(advancedOptions, SIGNAL(clicked()), this, SLOT(slotAdvClicked()));
        advancedOptions->setVisible(((ONMainWindow *) parent)->getShowAdvOption());
        advOptionsShown = false;
        conWidg = new ConnectionWidget(QString(), (ONMainWindow *) parent, this);
        setWidg = new SettingsWidget(QString(), (ONMainWindow *) parent, this);
        mediaWidget = new MediaWidget(QString(), (ONMainWindow *) parent, this);
        conWidg->hide();
        setWidg->hide();
        mediaWidget->hide();

        X2goSettings st("sessions");
        cbStartEmbed->setChecked(st.setting()->value("embedded/startembed", true).toBool());
    }
    /* #ifdef Q_OS_WIN
      else
      {
...
#ifdef USELDAP
    if (!embedMode) {
        st.setting()->setValue("LDAP/useldap", (QVariant) gbLDAP->isChecked());
        st.setting()->setValue("LDAP/port", (QVariant) port->value());
        if (ldapServer->text().length())
            st.setting()->setValue("LDAP/server", (QVariant) ldapServer->text());
        st.setting()->setValue("LDAP/port1", (QVariant) port1->value());
        if (ldapServer1->text().length())
            st.setting()->setValue("LDAP/server1", (QVariant) ldapServer1->text());
        st.setting()->setValue("LDAP/port2", (QVariant) port2->value());
        if (ldapServer2->text().length())
            st.setting()->setValue("LDAP/server2", (QVariant) ldapServer2->text());
        if (ldapBase->text().length())
            st.setting()->setValue("LDAP/basedn", (QVariant) ldapBase->text());
    }
#endif // USELDAP
#ifdef Q_OS_DARWIN
    st.setting()->setValue("xdarwin/directory", (QVariant) leXexec->text());
#endif

    pwid->saveSettings();
    if (embedMode) {
        X2goSettings st("sessions");
        st.setting()->setValue("embedded/startembed", (QVariant) cbStartEmbed->isChecked());
        st.setting()->sync();
...
void ConfigDialog::slot_checkOkStat()
{
    ok->setEnabled((!gbLDAP->isChecked())
                   || ((ldapBase->text().length() && ldapServer->text().length())));
}
    ...
    if (path.isEmpty()) {
        ...
        for (const_it = locations.constBegin(); const_it != locations.constEnd(); ++const_it) {
            ...
            if (info_plist.exists()) {
                ...
                versions << settings.value("CFBundleShortVersionString", (QVariant) "0.0.0")
                                .toString();
            } else {
...
void ConfigDialog::printXDarwinVersionWarning(QString version)
{
    show_RichText_WarningMsgBox(
        tr("Your XQuartz version is too old."),
        tr("Your are using XQuartz (X Window System Server for OS X) version %1.").arg(version)
            ...
            + tr("This version causes problems with X applications in 24bit "
                 ...
                 "<center><a "
                 "href=\"https://guide.macports.org/chunked/"
...
void ConfigDialog::slot_selectXDarwin()
{
    ...
    if (newDir.length() > 0) {
        ...
        if (version.compare(QString("0.0.0")) == 0) {
            show_RichText_WarningMsgBox(
                tr("No valid XQuartz application selected."),
...
{
    X2goSettings st("settings");
    return st.setting()->value("xdarwin/directory", (QVariant) "").toString();
}
#endif
