...
SVGFrame::SVGFrame(QString fname, bool st, QWidget *parent, Qt::WindowFlags f)
    : QFrame(parent, f)
{
    empty = false;
    renderer = 0;
...
SVGFrame::SVGFrame(QWidget *parent, Qt::WindowFlags f)
    : QFrame(parent, f)
{
    repaint = false;
    empty = true;
    ...
    /*	if ( isVisible() )
          {
                  ...
                  int barHeight;
                  if ( parentWidget )
                          wapiGetBorders ( parentWidget->winId(), vBorder,
     hBorder, barHeight ); x2goDebug<<"svg frame: "<<event->pos() <<
                  ":"<<mapFromGlobal ( event->pos() )
     <<":"<<barHeight<<":"<<vBorder<<":"
                  <<hBorder<<":"<<pos() <<Qt::endl;
                  QMouseEvent * nevent=new QMouseEvent(event->type(),
     QPoint(0,0), event->button(), event-> buttons(), event->modifiers());
