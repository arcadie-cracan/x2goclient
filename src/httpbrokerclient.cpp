...
HttpBrokerClient::HttpBrokerClient(ONMainWindow *wnd, ConfigFile *cfg)
{
    ...
    if (config->brokerurl.indexOf("ssh://") == 0) {
        ...
        if ((config->brokerCaCertFile.length() > 0) && (QFile::exists(config->brokerCaCertFile))) {
            ...
            x2goDebug << "Custom CA certificate file loaded into HTTPS broker client: "
                      << config->brokerCaCertFile;
        }
...
{
    QUrl lurl(config->brokerurl);
    sshConnection = new SshMasterConnection(this,
                                            lurl.host(),
                                            lurl.port(22),
                                            mainWindow->getAcceptRSA(),
                                            config->brokerUser,
                                            config->brokerPass,
                                            config->brokerSshKey,
                                            config->brokerAutologin,
                                            config->brokerKrbLogin,
                                            false);
    ...
    connect(sshConnection,
            ...
            this,
            SLOT(slotSshUserAuthError(QString)));
    connect(sshConnection,
    ...
    connect(mainWindow->getInteractionDialog(),
...
void HttpBrokerClient::slotSshServerAuthError(int error,
                                              QString sshMessage,
                                              SshMasterConnection *connection)
{
    ...
    if (QMessageBox::warning(0, tr("Host key verification failed."), errMsg, tr("Yes"), tr("No"))
        != 0) {
        connection->writeKnownHosts(false);
        connection->wait();
        if (sshConnection && sshConnection != connection) {
...
void HttpBrokerClient::slotSshServerAuthPassphrase(
    SshMasterConnection *connection, SshMasterConnection::passphrase_types passphrase_type)
{
    bool ok;
    QString message;
...
void HttpBrokerClient::getUserSessions()
{
    ...
    // Otherwise, after logout from the session, we will be connected by a
    // previous user without a password by authid.
    ...
    if (!sshBroker) {
        QString req;
        QTextStream(&req) << "task=listsessions&"
        ...
        if (nextAuthId.length() > 0) {
            sshConnection->executeCommand(config->sshBrokerBin + " --user " + brokerUser
                                              + " --authid " + nextAuthId + " --task listsessions",
                                          this,
...
void HttpBrokerClient::selectUserSession(const QString &session, const QString &loginName)
{
    ...
    if (!sshBroker) {
        QString req;
        QTextStream(&req) << "task=selectsession&"
                          << "sid=" << session << "&"
                          << "user=" << QUrl::toPercentEncoding(brokerUser) << "&"
                          << "password=" << QUrl::toPercentEncoding(config->brokerPass) << "&"
                          << "authid=" << nextAuthId;
...
void HttpBrokerClient::sendEvent(const QString &ev,
                                 const QString &id,
                                 ...
                                 uint connectionTime)
{
    ...
    if (!sshBroker) {
        QString req;
        QTextStream(&req) << "task=clientevent&"
                          << "user=" << QUrl::toPercentEncoding(brokerUser) << "&"
                          << "password=" << QUrl::toPercentEncoding(config->brokerPass) << "&"
                          << "sid=" << id << "&"
                          << "event=" << ev << "&"
        ...
        if (nextAuthId.length() > 0) {
            sshConnection->executeCommand(config->sshBrokerBin + " --user " + brokerUser
                                              + " --authid " + nextAuthId
                                              + " --task clientevent --sid \"" + id + "\" --event "
                                              + ev + " --server \"" + server + "\" --client \""
                                              + client + "\" --login " + "\"" + login
                                              + "\" --cmd \"" + cmd + "\" --display \"" + display
                                              + "\" --start \"" + start + "\" --elapsed "
                                              + QString::number(connectionTime),
                                          this,
                                          SLOT(slotEventSent(bool, QString, int)));
        } else {
            sshConnection->executeCommand(config->sshBrokerBin + " --user " + brokerUser
                                              + " --task clientevent --sid \"" + id + "\" --event "
                                              + ev + " --server \"" + server + "\" --client \""
...
void HttpBrokerClient::testConnection()
{
    ...
    if (!sshBroker) {
        ...
        if (nextAuthId.length() > 0) {
            sshConnection->executeCommand(config->sshBrokerBin + " --authid " + nextAuthId
                                              + " --task testcon",
                                          this,
                                          SLOT(slotSelectSession(bool, QString, int)));
        } else {
...
void HttpBrokerClient::processClientConfig(const QString &raw_content)
{
    ...
    if (mainWindow->config.brokerEvents) {
        ...
        if (mainWindow->config.brokerLiveEventsTimeout) {
            x2goDebug << "sending alive events to broker every "
                      << mainWindow->config.brokerLiveEventsTimeout << " seconds";
        }
    }
    if (mainWindow->config.brokerSyncTimeout) {
        x2goDebug << "synchronizing with broker every " << mainWindow->config.brokerSyncTimeout
                  << " seconds";
    }
}
...
void HttpBrokerClient::slotConnectionTest(bool success, QString answer, int)
{
    ...
    if (!checkAccess(answer))
        return;
    if (!sshBroker) {
        x2goDebug << "Elapsed: " << requestTime.elapsed() << "; received:" << answer.size()
                  << Qt::endl;
...
void HttpBrokerClient::parseSession(QString sinfo)
{
    ...
    QStringList words = serverLine.split(":", Qt::SkipEmptyParts);
...
void HttpBrokerClient::slotSslErrors(QNetworkReply *netReply, const QList<QSslError> &errors)
{
    ...
    if (QFile::exists(homeDir + "/.x2go/ssl/exceptions/" + lurl.host() + "/" + fname)) {
        QFile fl(homeDir + "/.x2go/ssl/exceptions/" + lurl.host() + "/" + fname);
    ...
    QMessageBox mb(QMessageBox::Warning, tr("Secure connection failed."), text);
    text = QString();
    QTextStream(&text) << err.join("\n") << "\n"
...
#endif
                       << Qt::endl
                       << tr("Serial Number\t") << getHexVal(cert.serialNumber()) << Qt::endl
                       << Qt::endl
                       << tr("Issued by:\n") << tr("Common Name(CN)\t") <<
#if QT_VERSION >= 0x050000
        ...
        tr("Validity:\n") << tr("Issued on\t") << cert.effectiveDate().toString() << Qt::endl
                       << tr("expires on\t") << cert.expiryDate().toString() << Qt::endl
                       << Qt::endl
                       << tr("Fingerprints:\n") << tr("SHA1\t")
    ...
    mb.setEscapeButton(
        (QAbstractButton *) mb.addButton(tr("Exit X2Go Client"), QMessageBox::RejectRole));
    QPushButton *okButton = mb.addButton(tr("Add exception"), QMessageBox::AcceptRole);
    ...
    if (mb.clickedButton() == (QAbstractButton *) okButton) {
        ...
        fl.close();
        netReply->ignoreSslErrors();
        x2goDebug << "Storing certificate in "
                  << homeDir + "/.x2go/ssl/exceptions/" + lurl.host() + "/" + fname;
        requestTime.restart();
    } else
        emit fatalHttpError();
}
...
QString HttpBrokerClient::getHexVal(const QByteArray &ba)
{
    ...
    for (int i = 0; i < ba.size(); ++i) {
        QString bt;
        bt.asprintf("%02X", (unsigned char) ba[i]);
        val << bt;
    }
    return val.join(":");
}
