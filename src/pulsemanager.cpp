/***************************************************************************
 *  Copyright (C) 2012-2020 by Mihai Moldovan <ionic@ionic.de>             *
 *                                                                         *
    ...
    , shutdown_state_(false)
{
    ...
    if (server_binary_.isEmpty()) {
        ...
        if (server_binary_.isEmpty()) {
            ...
            if (server_binary_.isEmpty()) {
                search_paths = QStringList();
                search_paths << "/usr/local/bin"; /* Homebrew or random stuff. Probably
                                             even both intermingled... */
    ...
    for (buf = ptr = NULL; ptr == NULL; path_len += 20) {
        if (NULL == (buf = static_cast<char *>(realloc(buf, path_len)))) {
            ...
            emit sig_pulse_user_warning(true,
                                        tr("Could not allocate buffer for getting "
...
void PulseManager::start_generic()
{
     ...
     */
        if (pulse_server_->waitForStarted(-1)) {
            ...
            connect(pulse_server_, SIGNAL(finished(int)), this, SLOT(slot_on_pulse_finished(int)));
...
void PulseManager::start_osx()
{
    ...
    if (!system_pulse_) {
        ...
        server_args_ << "-p"
                     << QDir::toNativeSeparators(QDir(app_dir_ + "/../Frameworks/pulse-"
    ...
    if (debug_) {
        server_args_ << "--log-level=debug"
                     << "--verbose"
                     << "--log-target=file:"
                            + QDir::toNativeSeparators(pulse_dir_.absolutePath() + "/pulse.log");
    }
 ...
 */
    ...
    server_args_ << "--exit-idle-time=-1"
                 ...
                 << QDir::toNativeSeparators(
                        QDir(pulse_dir_.absolutePath() + "/config.pa").absolutePath())
                 << "-p"
                 << QDir::toNativeSeparators(
                        QDir(app_dir_ + "/pulse/lib/pulse-" + QString::number(pulse_version_major_)
                             + "." + QString::number(pulse_version_minor_) + "/modules/")
                            .absolutePath());
    if (debug_) {
        server_args_ << "--log-level=debug"
                     << "--verbose"
...
void PulseManager::fetch_pulseaudio_version()
{
    ...
    if (tmp_server.waitForFinished()) {
        ...
        bool found = false;
        for (QStringList::const_iterator cit = stdout_list.begin();
             (cit != stdout_list.end()) && (!stop_processing);
             ++cit) {
            ...
            if (tmp_str.startsWith(needle)) {
                ...
                QString numbers[3] = {};
                for (QString::const_iterator cit = tmp_str.begin();
                     (cit != tmp_str.end()) && (!stop_processing);
                     ++cit) {
               ...
               */
                            ...
                            x2goErrorf(21) << "Unexpected character found when parsing "
                                              "version string for major version number: '"
                                           << QString(*cit) << "'.";
                            emit sig_pulse_user_warning(
                                true,
                                tr("Error fetching PulseAudio version number!"),
                                tr("Unexpected character found when parsing version string "
                        ...
                        if (((*cit) >= '0') && ((*cit) <= '9')) {
                            ...
                            /* Second number part complete and no more numbers, mark as done,
               * and... */
                            numbers_finished[1] = true;
                            ...
                            x2goErrorf(23) << "Unexpected character found when parsing "
                                              "version string for minor version number: '"
                                           << QString(*cit) << "'.";
                            emit sig_pulse_user_warning(
                                true,
                                tr("Error fetching PulseAudio version number!"),
                                tr("Unexpected character found when parsing version string "
                                   "for minor version number")
                                    + ": '" + QString(*cit) + "'.",
                                true);
                            stop_processing = true;
                        }
               ...
               */
                            ...
                            x2goErrorf(25) << "Unexpected character found when parsing "
                                              "version string for micro version number: '"
                                           << QString(*cit) << "'.";
                            emit sig_pulse_user_warning(
                                true,
                                tr("Error fetching PulseAudio version number!"),
                                tr("Unexpected character found when parsing version string "
                                   "for micro version number")
                                    + ": '" + QString(*cit) + "'.",
                                true);
                            stop_processing = true;
                        }
         ...
         * convert and met EOL)) */
                if ((!numbers_skip[0]) && ((numbers_finished[0]) || (numbers_started[0]))) {
                    ...
                    if (!convert_success) {
                        x2goErrorf(20)
                            << "Unable to convert major version number string to integer.";
                        emit sig_pulse_user_warning(
                            true,
                            tr("Error fetching PulseAudio version number!"),
         ...
         * convert and met EOL)) */
                if ((!numbers_skip[1]) && ((numbers_finished[1]) || (numbers_started[1]))) {
                    ...
                    if (!convert_success) {
                        x2goErrorf(22)
                            << "Unable to convert minor version number string to integer.";
                        emit sig_pulse_user_warning(
                            true,
                            tr("Error fetching PulseAudio version number!"),
         ...
         * convert and met EOL)) */
                if ((!numbers_skip[2]) && ((numbers_finished[2]) || (numbers_started[2]))) {
                    ...
                    if (!convert_success) {
                        x2goErrorf(24)
                            << "Unable to convert micro version number string to integer.";
                        emit sig_pulse_user_warning(
                            true,
                            tr("Error fetching PulseAudio version number!"),
...
bool PulseManager::generate_server_config()
{
     ...
     */
        ...
        config_tmp_file_stream << "load-module module-esound-protocol-tcp port="
                               << QString::number(esd_port_) << Qt::endl;
...
{
    QTemporaryFile client_config_tmp_file(pulse_dir_.absolutePath() + "/tmp/tmpconfig");
...
void PulseManager::cleanup_client_dir()
{
    ...
    // Delete it first.
    QDir machine_dir(pulse_dir_.absolutePath() + "/.pulse/" + QHostInfo::localHostName()
                     + "-runtime");
...
#ifdef Q_OS_WIN
    work_dir.remove(pulse_dir_.absolutePath() + "/pulse-pulseuser/pid");
    work_dir.rmdir(pulse_dir_.absolutePath() + "/pulse-pulseuser");
#endif // defined (Q_OS_WIN)

#if defined(Q_OS_DARWIN) || defined(Q_OS_WIN)
    // Remove server config file, otherwise the directory won't be empty.
    if (!debug_) {
        work_dir.remove(QDir::toNativeSeparators(
            QDir(pulse_dir_.absolutePath() + "/config.pa").absolutePath()));
        work_dir.remove(QDir::toNativeSeparators(
            QDir(pulse_dir_.absolutePath() + "/pulse.log").absolutePath()));
    }
#else // Linux
    // FIXME.
#endif
...
void PulseManager::shutdown()
{
    ...
    connect(this, SIGNAL(sig_pulse_server_terminated()), &loop, SLOT(quit()));
...
void PulseManager::show_startup_warning(bool play_startup_sound)
{
    ...
    informative_text += tr("If you downloaded the bundled, pre-compiled version from the "
                           "official home page "
